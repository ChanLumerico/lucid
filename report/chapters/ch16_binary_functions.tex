...
\begin{itemize}
  ...
  \item \code{floordiv(a, b)} computes $\lfloor a / b \rfloor$;
    integer division. Non-differentiable.
  \item \code{mod(a, b)} computes $a - b \cdot \lfloor a/b \rfloor$;
    remainder. Non-differentiable.
  \item \code{atan2(y, x)} computes $\arctan(y/x)$ with quadrant;
    backward: $\partial_y = x/(x^2 + y^2) \cdot \text{grad\_out}$,
    $\partial_x = -y/(x^2 + y^2) \cdot \text{grad\_out}$.
