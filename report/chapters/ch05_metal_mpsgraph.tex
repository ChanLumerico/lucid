...
The summary: \code{MTLStorageModeShared} for CPU/GPU shared
access, \code{MTLStorageModePrivate} for GPU-only, with two
other modes (\emph{Managed}, \emph{Memoryless}) that \Lucid\
never encounters. \MLX\ allocates all of its arrays in
\code{MTLStorageModeShared}, which is what makes \Lucid's
zero-copy bridge possible.
