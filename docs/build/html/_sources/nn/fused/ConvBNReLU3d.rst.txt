...
.. autoclass:: lucid.nn.ConvBNReLU3d

The `ConvBNReLU3d` module combines a 3D convolutional layer, 
batch normalization, and ReLU activation in one sequential block. 
It is particularly useful for processing 3D data such as volumetric data or video inputs.
...
- **padding** (*_PaddingStr | int | tuple[int, int, int]*, optional):
  Padding added to all sides of the input. Default is 0.

- **dilation** (*int | tuple[int, int, int]*, optional):
