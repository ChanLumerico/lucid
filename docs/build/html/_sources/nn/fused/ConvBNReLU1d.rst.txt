...
.. autoclass:: lucid.nn.ConvBNReLU1d

The `ConvBNReLU1d` module combines a 1D convolutional layer, 
batch normalization, and ReLU activation in one sequential block. 
It is particularly useful for processing 1D signals such as time series or sequential data.
...
.. math::
    ...
    \text{output} = \text{ReLU}(\text{BatchNorm}(\text{Conv1D}(\text{input})))
...
.. code-block:: python
    ...
    ...     padding=1
    ... )
    >>> input_ = Tensor([[1.0, 2.0, 3.0, 4.0]])  # Shape: (N, C_in, L)
    >>> output = layer(input_)
    >>> print(output)
