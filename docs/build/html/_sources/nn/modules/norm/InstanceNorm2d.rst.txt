...
.. autoclass:: lucid.nn.InstanceNorm2d

The `InstanceNorm2d` module applies Instance Normalization over a two-dimensional input 
(a batch of 2D inputs with optional additional dimensions). 
...
This normalization technique is particularly useful in tasks like image generation and 
style transfer, where per-instance normalization can lead to better performance 
...
- **num_features** (*int*):
    - The number of features or channels in the input tensor. 
      For a 2D input, this typically corresponds to the number of channels.
...
Forward Calculation
-------------------
The `InstanceNorm2d` module normalizes the input tensor and optionally applies 
...
- :math:`\mathbf{x}` is the input tensor of shape :math:`(N, C, H, W)` where:
  - :math:`N` is the batch size.
  - :math:`C` is the number of channels/features.
  - :math:`H` and :math:`W` are the height and width of the input.

- :math:`\mu` is the mean of the input over the spatial dimensions :math:`H` and :math:`W` for each instance and channel.
- :math:`\sigma^2` is the variance of the input over the spatial dimensions :math:`H` and :math:`W` for each instance and channel.
...
.. math::
    ...
    \frac{\gamma}{\sqrt{\sigma^2 + \epsilon}} \left( \mathbf{1} - \frac{1}{H \cdot W} \right) 
...
.. math::
    ...
    \sum_{i=1}^{N} \sum_{j=1}^{H} \sum_{k=1}^{W} \mathbf{y}_{i,j,k} \cdot 
    \left( \frac{\mathbf{x}_{i,j,k} - \mu_i}{\sqrt{\sigma^2_i + \epsilon}} \right)
...
.. math::
    ...
    \sum_{i=1}^{N} \sum_{j=1}^{H} \sum_{k=1}^{W} \mathbf{y}_{i,j,k}
