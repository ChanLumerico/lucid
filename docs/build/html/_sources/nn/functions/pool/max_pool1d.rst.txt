...
.. autofunction:: lucid.nn.functional.max_pool1d

The `max_pool1d` function performs a one-dimensional max pooling operation on the input tensor.
...
- **Tensor**: 
    The result of the 1D max pooling operation, with shape (N, C, L_out), where:
...
Performing a simple 1D max pooling:

.. code-block:: python

    >>> import lucid.nn.functional as F
    >>> input_ = Tensor([[[1.0, 2.0, 3.0, 4.0]]])  # Shape: (1, 1, 4)
    >>> out = F.max_pool1d(input_, kernel_size=2, stride=2, padding=0)
    >>> print(out)
    Tensor([[[2.0, 4.0]]])
