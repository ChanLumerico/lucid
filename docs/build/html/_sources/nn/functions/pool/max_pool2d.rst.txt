...
.. autofunction:: lucid.nn.functional.max_pool2d

The `max_pool2d` function performs a two-dimensional max pooling operation on the input tensor.
...
- **Tensor**: 
    The result of the 2D max pooling operation, with shape (N, C, H_out, W_out), where:
...
Performing a simple 2D max pooling:

.. code-block:: python

    >>> import lucid.nn.functional as F
    >>> input_ = Tensor([[[[1.0, 2.0], [3.0, 4.0]]]])  # Shape: (1, 1, 2, 2)
    >>> out = F.max_pool2d(input_, kernel_size=2, stride=1, padding=0)
    >>> print(out)
    Tensor([[[[4.0]]]])
