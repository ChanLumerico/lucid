...
The `swin_base` function instantiates a base Swin Transformer model with 
...
- **kwargs** (*dict*, optional):
  ...
  - **embed_dim** (*int*):
    ...
    The typical default for the base model is 128.
  ...
  - **num_heads** (*list[int]*):
    ...
    The common default for the base model is `[4, 8, 16, 32]`, corresponding 
    to the number of heads used in each stage, which enables the model to 
    capture multi-scale contextual information.

Returns
-------
- **SwinTransformer**:
  An instance of the `SwinTransformer` class configured as a base vision transformer.
