...
.. autofunction:: lucid.models.detr_r101

The `detr_r101` function returns an instance of the `DETR` model
configured with a **ResNet-101** backbone.

**Total Parameters**: 60,570,528 (MS-COCO)
...
- **pretrained_backbone** (*bool*, default=False): 
  If `True`, initializes the ResNet-101 backbone with pretrained weights and (by default) 
  **fine-tunes** it.

- **kwargs**:
  Additional keyword arguments passed to `DETRConfig`. This factory fixes the
  ResNet-101 backbone preset and the default DETR transformer dimensions.

Returns
-------
- **DETR**:
  
  A DETR-R101 detector with class and box heads. 
...
.. code-block:: python
    ...
    >>> from lucid.models import detr_r101
    >>> import lucid
    >>>
    >>> # COCO-style config (91 classes), 100 queries
    >>> model = detr_r101(num_classes=91, num_queries=100, pretrained_backbone=True, aux_loss=False)
    >>> print(model)
    >>>
    >>> # Dummy input
