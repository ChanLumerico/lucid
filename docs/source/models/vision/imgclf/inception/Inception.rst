...
It now takes an `InceptionConfig` object that centralizes the selected variant,
class count, input channel count, auxiliary classifier setting, and optional
dropout override.
...
- **config** (*InceptionConfig*)
  A configuration object describing the Inception variant, output class count,
  input channels, auxiliary classifier usage, and optional dropout override.
