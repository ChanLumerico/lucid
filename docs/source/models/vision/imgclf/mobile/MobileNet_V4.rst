...
for mobile and embedded applications. Model structure is defined through
`MobileNetV4Config`, which stores the dictionary-based block specification used to
assemble the conv and hybrid variants.
...
- **config** (*MobileNetV4Config*):
  Configuration object describing the stage specification dictionary and classifier size
  for the MobileNet-v4 family.
