...
handling the registration of submodules and parameters, and maintaining their state during training and evaluation.

The `Module` class serves as a base for defining custom neural network layers, 
enabling functionality like forward propagation, parameter management, and serialization of model states.
...
Dynamically manages the addition of `Parameter` or `Module` objects as attributes to the module. 
...
.. code-block:: python
    ...
    def load_state_dict(self, state_dict: dict[str, Parameter], strict: bool = True) -> None
...
Registers a hook to be executed before `forward`. The hook can optionally return
new inputs.
...
Registers a hook to be executed after `forward`. The hook can optionally return
a new output.
...
Registers a gradient hook on the output `Tensor` (only when the module returns
a single `Tensor`).
...
Registers a hook that runs when output gradients are ready, before full backward
hooks.
...
Registers a hook that runs with both input and output gradients.
...
- `grad_input_tuple` is aligned with positional `args` only. Non-`Tensor` inputs
  are `None`. Keyword-only inputs are not included.
- `grad_output_tuple` contains gradients for each `Tensor` in the output (non-
  `Tensor` outputs are skipped).
- All hook registration methods return a callable that removes the hook.
...
.. warning:: **State dictionary mismatch**
   ...
   When loading a state dictionary, ensure the keys match the module's structure. 
   If `strict=True`, mismatched keys will raise an error.
