...
class TestGradcheckUnary:
    def test_exp(self):
        x = make_tensor((3,), low=-1.0, high=1.0, dtype=lucid.float64, requires_grad=True, seed=1)
        assert _gc(lambda t: lucid.sum(lucid.exp(t)), (x,))

    def test_log(self):
        x = make_tensor((3,), low=0.5, high=2.0, dtype=lucid.float64, requires_grad=True, seed=2)
        assert _gc(lambda t: lucid.sum(lucid.log(t)), (x,))

    def test_sin(self):
        x = make_tensor((3,), low=-1.0, high=1.0, dtype=lucid.float64, requires_grad=True, seed=3)
        assert _gc(lambda t: lucid.sum(lucid.sin(t)), (x,))

    def test_relu(self):
        x = make_tensor((3,), low=0.2, high=2.0, dtype=lucid.float64, requires_grad=True, seed=4)
        assert _gc(lambda t: lucid.sum(lucid.relu(t)), (x,))

    def test_sigmoid(self):
        x = make_tensor((3,), low=-1.0, high=1.0, dtype=lucid.float64, requires_grad=True, seed=5)
