"""The recurrent layers, against themselves.
...
So the reference is the oracle for the values, and those checks live in
``lucid/test/parity/nn/test_recurrent_parity.py`` because they need it
installed; this file asserts the properties that do not need one — that
carrying the state across a split equals running the sequence whole.
"""
