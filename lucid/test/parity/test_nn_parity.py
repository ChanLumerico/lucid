...
class TestActivationParity:
    ...
    def test_gelu_none(self):
        l, t = _pair((4, 8))
        check_parity(LF.gelu(l, approximate="none"), TF.gelu(t, approximate="none"), atol=2e-4)

    def test_gelu_tanh(self):
        l, t = _pair((4, 8))
        check_parity(LF.gelu(l, approximate="tanh"), TF.gelu(t, approximate="tanh"), atol=2e-4)
...
class TestNormParity:
    ...
    def test_group_norm(self):
        ...
        l = lucid.tensor(x_np.copy())
        t = torch.tensor(x_np.copy())
        lw = lucid.ones(8); lb = lucid.zeros(8)
        tw = torch.ones(8); tb = torch.zeros(8)
        check_parity(LF.group_norm(l, lw, lb, num_groups=2),
                     TF.group_norm(t, 2, tw, tb), atol=2e-4)
...
class TestLinearParity:
    def test_linear_with_bias(self):
        ...
        l_out = LF.linear(lucid.tensor(x_np.copy()),
                          lucid.tensor(w_np.copy()),
                          lucid.tensor(b_np.copy()))
        t_out = TF.linear(torch.tensor(x_np.copy()),
...
class TestConvParity:
    def test_conv2d_basic(self):
        ...
        l_out = LF.conv2d(lucid.tensor(x_np.copy()),
                          lucid.tensor(w_np.copy()),
                          lucid.tensor(b_np.copy()),
                          padding=1)
        t_out = TF.conv2d(torch.tensor(x_np.copy()),
                          torch.tensor(w_np.copy()),
                          torch.tensor(b_np.copy()),
                          padding=1)
        check_parity(l_out, t_out)
...
class TestLossParity:
    def test_mse_loss(self):
        ...
        check_parity(LF.mse_loss(lucid.tensor(p.copy()), lucid.tensor(q.copy())),
    ...
    def test_cross_entropy(self):
        ...
        check_parity(
            LF.cross_entropy(lucid.tensor(logits.copy()),
                             lucid.tensor(targets.astype(np.int32))),
            TF.cross_entropy(torch.tensor(logits.copy()),
                             torch.tensor(targets.astype(np.int64))),
            atol=2e-4)
...
class TestSDPAParity:
    def test_sdpa_basic(self):
        ...
        l_out = LF.scaled_dot_product_attention(
            lucid.tensor(q_np.copy()), lucid.tensor(k_np.copy()), lucid.tensor(v_np.copy()))
        t_out = TF.scaled_dot_product_attention(
            torch.tensor(q_np.copy()), torch.tensor(k_np.copy()), torch.tensor(v_np.copy()))
