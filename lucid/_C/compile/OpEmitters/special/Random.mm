...
public:
    ...
    void* emit(BuilderContext& ctx, const OpNode& node) override {
        RngContext rc;
        if (!open_rng(ctx, node, rc)) return nullptr;
        ...
        d.min = 0.0f;
        d.max = 1.0f;
        return (__bridge void*)[rc.graph randomTensorWithShape:rc.ns_shape
                                                     descriptor:d
                                                           seed:rc.seed
                                                           name:@"rand"];
    }
...
public:
    ...
    void* emit(BuilderContext& ctx, const OpNode& node) override {
        ...
        if (rc.ns_dt == MPSDataTypeBool) {
            return (__bridge void*)mask_bool;
        }
        return (__bridge void*)[rc.graph castTensor:mask_bool
                                              toType:rc.ns_dt
                                                name:@"bernoulli"];
    }
};
