...
void register_optim(py::module_& m) {
    ...
    py::class_<SGD, Optimizer>(m, "SGD")
        .def(py::init<std::vector<std::shared_ptr<TensorImpl>>,
    ...
    py::class_<Adagrad, Optimizer>(m, "Adagrad")
        .def(py::init<std::vector<std::shared_ptr<TensorImpl>>,
             ...
             py::arg("params"), py::arg("lr") = 1e-2,
             py::arg("eps") = 1e-10, py::arg("weight_decay") = 0.0,
             py::arg("initial_accumulator_value") = 0.0,
             "Adagrad: per-parameter accumulator of squared grads.");

    py::class_<Adadelta, Optimizer>(m, "Adadelta")
        .def(py::init<std::vector<std::shared_ptr<TensorImpl>>,
                      double, double, double, double>(),
             py::arg("params"), py::arg("lr") = 1.0,
             py::arg("rho") = 0.9, py::arg("eps") = 1e-6,
             py::arg("weight_decay") = 0.0,
    ...
    py::class_<StepLR, LRScheduler>(m, "StepLR")
        .def(py::init<Optimizer&, std::int64_t, double>(),
    ...
    py::class_<LambdaLR, LRScheduler>(m, "LambdaLR")
        .def(py::init<Optimizer&, std::function<double(std::int64_t)>>(),
    ...
    cyclic.def(py::init<Optimizer&, double, double, std::int64_t,
                        std::int64_t, CyclicLR::Mode, double>(),
               py::arg("optimizer"),
               py::arg("base_lr"), py::arg("max_lr"),
               py::arg("step_size_up"),
               py::arg("step_size_down") = 0,
               py::arg("mode") = CyclicLR::Mode::Triangular,
               py::arg("gamma") = 1.0,
               py::keep_alive<1, 2>(),
               "Cyclic LR (Smith 2017): triangular wave between base_lr and max_lr.");

    py::class_<NoamScheduler, LRScheduler>(m, "NoamScheduler")
        .def(py::init<Optimizer&, std::int64_t, std::int64_t, double>(),
             py::arg("optimizer"),
             py::arg("model_size"), py::arg("warmup_steps"),
             py::arg("factor") = 1.0,
             py::keep_alive<1, 2>(),
             "Transformer-style warmup-then-decay schedule.");
    ...
    rlrp.def(py::init<Optimizer&, ReduceLROnPlateau::Mode, double,
                      std::int64_t, double, ReduceLROnPlateau::ThresholdMode,
        ...
        .def_property_readonly("num_bad_epochs",
                               &ReduceLROnPlateau::num_bad_epochs);
}
