...
void register_nn(py::module_& m) {
    ...
    m.def("layer_norm", &layer_norm_op,
    ...
    m.def("rms_norm", &rms_norm_op,
          py::arg("x"), py::arg("gamma"), py::arg("eps") = 1e-5,
    ...
    m.def(
        ...
        py::arg("a"), py::arg("p"), py::arg("training") = true,
    ...
    m.def(
        ...
        [](const TensorImplPtr& a, double p, bool training, py::object gen_obj) {
            Generator* gen = nullptr;
            if (!gen_obj.is_none()) gen = gen_obj.cast<Generator*>();
    ...
    m.def(
        "drop_block",
        [](const TensorImplPtr& a, std::int64_t block_size, double p,
           double eps, py::object gen_obj) {
            Generator* gen = nullptr;
            if (!gen_obj.is_none()) gen = gen_obj.cast<Generator*>();
            return drop_block_op(a, block_size, p, eps, gen);
        },
        py::arg("a"), py::arg("block_size"), py::arg("p") = 0.1,
        py::arg("eps") = 1e-7, py::arg("generator") = py::none(),
        "Spatial structured dropout (Ghiasi et al., 2018). Drops "
        "block_size×block_size square regions in 4-D feature maps.");

    m.def(
        "drop_path",
        [](const TensorImplPtr& a, double p, bool scale_by_keep,
           py::object gen_obj) {
            Generator* gen = nullptr;
            if (!gen_obj.is_none()) gen = gen_obj.cast<Generator*>();
    ...
    m.def("adaptive_max_pool1d", &adaptive_max_pool1d_op,
          py::arg("x"), py::arg("output_l"),
          "1D adaptive max pooling (uniform case: L must be divisible by output_l).");
    m.def("adaptive_max_pool2d", &adaptive_max_pool2d_op,
          py::arg("x"), py::arg("output_h"), py::arg("output_w"),
          "2D adaptive max pooling (uniform case).");
    m.def("adaptive_max_pool3d", &adaptive_max_pool3d_op,
          py::arg("x"), py::arg("output_d"), py::arg("output_h"), py::arg("output_w"),
          "3D adaptive max pooling (uniform case).");
    m.def("adaptive_avg_pool1d", &adaptive_avg_pool1d_op,
          py::arg("x"), py::arg("output_l"),
          "1D adaptive average pooling (uniform case).");
    m.def("adaptive_avg_pool2d", &adaptive_avg_pool2d_op,
          py::arg("x"), py::arg("output_h"), py::arg("output_w"),
          "2D adaptive average pooling (uniform case).");
    m.def("adaptive_avg_pool3d", &adaptive_avg_pool3d_op,
          py::arg("x"), py::arg("output_d"), py::arg("output_h"), py::arg("output_w"),
          "3D adaptive average pooling (uniform case).");

    m.def("max_pool1d", &max_pool1d_op,
          py::arg("x"), py::arg("kernel_l"),
          py::arg("stride_l") = 0, py::arg("pad_l") = 0,
          "1D max pooling. stride=0 means stride==kernel.");
    m.def("max_pool2d", &max_pool2d_op,
          py::arg("x"), py::arg("kernel_h"), py::arg("kernel_w"),
          py::arg("stride_h") = 0, py::arg("stride_w") = 0,
          py::arg("pad_h") = 0, py::arg("pad_w") = 0,
          "2D max pooling. stride=0 means stride==kernel.");
    m.def("max_pool3d", &max_pool3d_op,
          py::arg("x"), py::arg("kernel_d"), py::arg("kernel_h"), py::arg("kernel_w"),
          py::arg("stride_d") = 0, py::arg("stride_h") = 0, py::arg("stride_w") = 0,
          py::arg("pad_d") = 0, py::arg("pad_h") = 0, py::arg("pad_w") = 0,
          "3D max pooling. stride=0 means stride==kernel.");

    m.def("avg_pool1d", &avg_pool1d_op,
          py::arg("x"), py::arg("kernel_l"),
          py::arg("stride_l") = 0, py::arg("pad_l") = 0,
          "1D average pooling. stride=0 means stride==kernel.");
    m.def("avg_pool2d", &avg_pool2d_op,
          py::arg("x"), py::arg("kernel_h"), py::arg("kernel_w"),
          py::arg("stride_h") = 0, py::arg("stride_w") = 0,
          py::arg("pad_h") = 0, py::arg("pad_w") = 0,
          "2D average pooling. stride=0 means stride==kernel.");
    m.def("avg_pool3d", &avg_pool3d_op,
          py::arg("x"), py::arg("kernel_d"), py::arg("kernel_h"), py::arg("kernel_w"),
          py::arg("stride_d") = 0, py::arg("stride_h") = 0, py::arg("stride_w") = 0,
          py::arg("pad_d") = 0, py::arg("pad_h") = 0, py::arg("pad_w") = 0,
          "3D average pooling. stride=0 means stride==kernel.");

    m.def("batch_norm1d", &batch_norm1d_op,
          py::arg("x"), py::arg("gamma"), py::arg("beta"),
          py::arg("eps") = 1e-5,
          "Pure-function BatchNorm1d. x:(B,C,L). γ,β:(C,).");
    m.def("batch_norm", &batch_norm_op,
          py::arg("x"), py::arg("gamma"), py::arg("beta"),
          py::arg("eps") = 1e-5,
          "Pure-function BatchNorm2d (train-mode statistics, no running stats). "
          "x:(B,C,H,W). γ,β:(C,). Module wrapper handles running stats.");
    m.def("batch_norm3d", &batch_norm3d_op,
          py::arg("x"), py::arg("gamma"), py::arg("beta"),
          py::arg("eps") = 1e-5,
          "Pure-function BatchNorm3d. x:(B,C,D,H,W). γ,β:(C,).");

    m.def("group_norm", &group_norm_op,
          py::arg("x"), py::arg("gamma"), py::arg("beta"),
    ...
    m.def("batch_norm_eval", &batch_norm_eval_op,
          py::arg("x"), py::arg("mean"), py::arg("var"),
          py::arg("gamma"), py::arg("beta"), py::arg("eps") = 1e-5,
          "Inference-mode BatchNorm using precomputed running stats.");

    m.def("lp_normalize", &lp_normalize_op,
          py::arg("x"), py::arg("ord"), py::arg("axis"), py::arg("eps") = 1e-12,
          "Lp normalize: y = x / max(||x||_p, eps) along `axis`.");

    m.def("global_response_norm", &global_response_norm_op,
          py::arg("x"), py::arg("gamma"), py::arg("beta"), py::arg("eps") = 1e-6,
          "ConvNeXt-v2 GRN: gamma·(x·Nx) + beta·x with Nx = ||x||_2 / mean.");

    // ---------- Loss kernels ----------
    // reduction:  0 = None, 1 = Mean, 2 = Sum
    m.def("mse_loss", &mse_loss_op,
          py::arg("input"), py::arg("target"), py::arg("reduction") = 1,
          "MSE loss. reduction: 0=None, 1=Mean, 2=Sum.");
    m.def("bce_loss", &bce_loss_op,
          py::arg("input"), py::arg("target"),
          py::arg("weight") = TensorImplPtr{},
          py::arg("reduction") = 1, py::arg("eps") = 1e-7,
          "Binary cross-entropy. Input must be in [0, 1].");
    m.def("bce_with_logits", &bce_with_logits_op,
          py::arg("input"), py::arg("target"),
          py::arg("weight") = TensorImplPtr{},
          py::arg("pos_weight") = TensorImplPtr{},
          py::arg("reduction") = 1,
          "Binary cross-entropy with sigmoid fused into the loss for stability.");
    m.def("cross_entropy_loss", &cross_entropy_op,
          py::arg("input"), py::arg("target"),
          py::arg("weight") = TensorImplPtr{},
          py::arg("reduction") = 1, py::arg("eps") = 1e-7,
          py::arg("ignore_index") = -100,
          "Cross-entropy = LogSoftmax + NLL fused.");
    m.def("nll_loss", &nll_loss_op,
          py::arg("input"), py::arg("target"),
          py::arg("weight") = TensorImplPtr{},
          py::arg("reduction") = 1, py::arg("ignore_index") = -100,
          "Negative log-likelihood loss; input must be log-probabilities.");
    m.def("huber_loss", &huber_loss_op,
          py::arg("input"), py::arg("target"),
          py::arg("delta") = 1.0, py::arg("reduction") = 1,
          "Huber loss with parameter `delta`.");

    // ---------- Attention ----------
    m.def("scaled_dot_product_attention", &scaled_dot_product_attention_op,
          py::arg("query"), py::arg("key"), py::arg("value"),
          py::arg("attn_mask") = TensorImplPtr{},
    ...
    m.def(
        ...
        [](const TensorImplPtr& q, const TensorImplPtr& k, const TensorImplPtr& v,
           const TensorImplPtr& attn_mask, double scale, bool is_causal) {
            auto r = scaled_dot_product_attention_with_weights_op(
                q, k, v, attn_mask, scale, is_causal);
    ...
    // ---------- Embedding family ----------
    m.def("embedding", &embedding_op,
          py::arg("weight"), py::arg("indices"),
    ...
    m.def("sinusoidal_pos_embedding", &sinusoidal_pos_embedding_op,
          py::arg("seq_len"), py::arg("embed_dim"),
          py::arg("dtype") = Dtype::F32, py::arg("device") = Device::CPU,
          "Standard sinusoidal positional table — no autograd.");

    m.def("rotary_pos_embedding", &rotary_pos_embedding_op,
          py::arg("input"),
          py::arg("position_ids") = TensorImplPtr{},
    ...
    // ---------- Spatial transformer ----------
    m.def("affine_grid", &affine_grid_op,
          py::arg("theta"), py::arg("N"), py::arg("H"), py::arg("W"),
          py::arg("align_corners") = true,
          "Builds a sampling grid from (N, 2, 3) affine matrices. "
          "Output shape: (N, H, W, 2).");

    m.def("grid_sample", &grid_sample_op,
          py::arg("input"), py::arg("grid"),
          py::arg("mode") = 0, py::arg("padding_mode") = 0,
          py::arg("align_corners") = true,
