...
void register_utils(py::module_& m) {
    ...
    m.def("split", &split_op,
          py::arg("a"), py::arg("num_splits"), py::arg("axis") = 0);
    m.def("split_at",
          [](const TensorImplPtr& a, std::vector<std::int64_t> indices,
             int axis) { return split_at_op(a, std::move(indices), axis); },
          py::arg("a"), py::arg("indices"), py::arg("axis") = 0);
    m.def("chunk", &chunk_op, py::arg("a"), py::arg("chunks"),
          py::arg("axis") = 0);
    m.def("unbind", &unbind_op, py::arg("a"), py::arg("axis") = 0);

    // ----- Repetition -----
    m.def("repeat", &repeat_op,
          py::arg("a"), py::arg("repeats"), py::arg("axis") = 0);
    m.def("tile",
    ...
    m.def("pad",
          [](const TensorImplPtr& a,
             ...
             double constant) {
              return pad_op(a, std::move(pad_width), constant);
          },
          py::arg("a"), py::arg("pad_width"), py::arg("constant") = 0.0);

    // ----- Layout transforms -----
    m.def("flatten", &flatten_op,
          py::arg("a"), py::arg("start_axis") = 0, py::arg("end_axis") = -1);
    m.def("broadcast_to",
    ...
    // ----- Selection / gathering -----
    m.def("where",       &where_op,
          py::arg("cond"), py::arg("x"), py::arg("y"));
    m.def("masked_fill", &masked_fill_op,
          py::arg("a"), py::arg("mask"), py::arg("value"));
    m.def("roll",
          [](const TensorImplPtr& a, std::vector<std::int64_t> shifts,
             std::vector<int> axes) {
              return roll_op(a, std::move(shifts), std::move(axes));
          },
          py::arg("a"), py::arg("shifts"), py::arg("axes"));
    m.def("gather", &gather_op,
          py::arg("a"), py::arg("indices"), py::arg("axis") = -1);
    m.def("diagonal", &diagonal_op,
          py::arg("a"), py::arg("offset") = 0,
          py::arg("axis1") = -2, py::arg("axis2") = -1);

    // ----- Sort / search -----
    m.def("sort",    &sort_op,    py::arg("a"), py::arg("axis") = -1);
    m.def("argsort", &argsort_op, py::arg("a"), py::arg("axis") = -1);
    m.def("argmax",  &argmax_op,
          py::arg("a"), py::arg("axis") = -1, py::arg("keepdims") = false);
    m.def("argmin",  &argmin_op,
          py::arg("a"), py::arg("axis") = -1, py::arg("keepdims") = false);
    ...
    // ----- Meshgrid -----
    m.def("meshgrid", &meshgrid_op,
          py::arg("arrays"), py::arg("indexing_xy") = false);
    ...
    m.def("ravel",
          ...
          py::arg("a"),
          "Flatten to 1-D (alias for reshape(a, [-1])).");

    // ----- Histogram -----
    m.def("histogram",
          [](const TensorImplPtr& a, std::int64_t bins,
             double lo, double hi, bool density) {
              auto [c, e] = histogram_op(a, bins, lo, hi, density);
              return py::make_tuple(c, e);
          },
          py::arg("a"), py::arg("bins") = 10,
          py::arg("lo"), py::arg("hi"), py::arg("density") = false);
    ...
    m.def("histogram2d",
          [](const TensorImplPtr& a, const TensorImplPtr& b,
             ...
             bool density) {
              auto [c, e] = histogram2d_op(a, b, bins_a, bins_b,
                                            lo_a, hi_a, lo_b, hi_b, density);
              return py::make_tuple(c, e);
          },
          py::arg("a"), py::arg("b"),
          py::arg("bins_a"), py::arg("bins_b"),
          py::arg("lo_a"), py::arg("hi_a"),
          py::arg("lo_b"), py::arg("hi_b"),
    ...
    m.def("histogramdd",
          [](const TensorImplPtr& a,
             ...
             bool density) {
              auto [c, e] = histogramdd_op(a, std::move(bins),
                                            std::move(ranges), density);
              return py::make_tuple(c, e);
          },
          py::arg("a"), py::arg("bins"), py::arg("ranges"),
          py::arg("density") = false);
}
