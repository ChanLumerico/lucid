...
void register_quantized(py::module_& m) {
    m.def("quantize", &quantize_op, py::arg("w"), py::arg("group_size") = 64, py::arg("bits") = 4,
          "Group-wise quantize a float weight → [packed_weight, scales, biases].");
    m.def("dequantize", &dequantize_op, py::arg("w"), py::arg("scales"),
          py::arg("biases") = TensorImplPtr{}, py::arg("group_size") = 64, py::arg("bits") = 4,
          "Reconstruct a float weight from its packed form.");
    m.def("quantized_matmul", &quantized_matmul_op, py::arg("x"), py::arg("w"), py::arg("scales"),
          py::arg("biases") = TensorImplPtr{}, py::arg("transpose") = true,
          py::arg("group_size") = 64, py::arg("bits") = 4,
