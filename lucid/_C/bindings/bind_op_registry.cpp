...
void register_op_registry(py::module_& m) {
    py::class_<OpSchema>(m, "OpSchema")
        .def_property_readonly("name",
                               [](const OpSchema& s) { return std::string(s.name); })
        ...
        .def_property_readonly("determinism_note", [](const OpSchema& s) {
            return std::string(s.determinism_note);
        })
