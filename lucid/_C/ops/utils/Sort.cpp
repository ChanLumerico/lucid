    ...
    for (std::size_t o = 0; o < outer; ++o) {
        for (std::size_t j = 0; j < inner; ++j) {
            ...
            auto cmp = [&](std::int32_t lhs, std::int32_t rhs) {
                const T lv = src[(o * L + static_cast<std::size_t>(lhs)) *
                                     inner +
                                 j];
                const T rv = src[(o * L + static_cast<std::size_t>(rhs)) *
                                     inner +
                                 j];
...
CpuStorage scatter_axis_add_cpu(const CpuStorage& grad,
                                ...
                                const Shape& input_shape,
                                const Shape& grad_shape, int axis, Dtype dt) {
    ...
    for (std::size_t o = 0; o < outer; ++o) {
        for (std::size_t j = 0; j < inner; ++j) {
            for (std::size_t k = 0; k < K; ++k) {
                ...
                if (src_k < 0) src_k += static_cast<std::int32_t>(L);
...
Storage scatter_axis_add_storage(const Storage& grad, const Storage& indices,
                                 const Shape& input_shape,
                                 const Shape& grad_shape, int axis, Dtype dt,
                                 Device device) {
    ...
    switch (dt) {
        case Dtype::F32:
            return Storage{scatter_axis_add_cpu<float>(
                g, idx, input_shape, grad_shape, axis, dt)};
        case Dtype::F64:
            return Storage{scatter_axis_add_cpu<double>(
                g, idx, input_shape, grad_shape, axis, dt)};
        default:
...
public:
    ...
    std::vector<Storage> apply(Storage grad_out) override {
        return {scatter_axis_add_storage(grad_out, indices_, input_shapes_[0],
                                         grad_shape_, axis_, dtype_, device_)};
    }
};

const OpSchema IndexScatterBackward::schema_v1{
    "index_scatter", 1, AmpPolicy::KeepInput, true};

TensorImplPtr attach_index_scatter_grad(const TensorImplPtr& a,
                                        TensorImplPtr out, Storage indices,
                                        int axis) {
    if (!GradMode::is_enabled() || !a->requires_grad_ ||
        !differentiable_dtype(a->dtype_)) {
        return out;
    }
...
::mlx::core::array take_descending_top_indices(const ::mlx::core::array& idx,
    ...
    for (std::int64_t i = 0; i < k; ++i)
        selector[static_cast<std::size_t>(i)] =
            static_cast<std::int32_t>(L - 1 - i);
    ::mlx::core::Shape selector_shape(full_shape.size(), 1);
    selector_shape[axis] = static_cast<int>(k);
    ::mlx::core::array selector_arr(selector.data(), selector_shape,
                                    ::mlx::core::int32);
...
TensorImplPtr sort_op(const TensorImplPtr& a, int axis) {
    if (!a) throw LucidError("sort: null input");
    ...
    if (device == Device::GPU) {
        ...
        auto out = fresh(Storage{gpu::wrap_mlx_array(std::move(out_raw), dt)},
                         a->shape_, dt, device);
        return attach_index_scatter_grad(
            a, std::move(out), Storage{gpu::wrap_mlx_array(std::move(idx),
                                                           Dtype::I32)},
            ax);
    }
    ...
    if (dt == Dtype::F32)
        ...
        std::tie(out_cpu, idx_cpu) = sort_select_cpu<std::int32_t>(
            ca, a->shape_, out_shape, ax, dt, false);
    else if (dt == Dtype::I64)
        std::tie(out_cpu, idx_cpu) = sort_select_cpu<std::int64_t>(
            ca, a->shape_, out_shape, ax, dt, false);
    else
        throw NotImplementedError("sort: dtype not supported");
    auto out = fresh(Storage{std::move(out_cpu)}, std::move(out_shape), dt,
                     device);
    return attach_index_scatter_grad(a, std::move(out),
                                     Storage{std::move(idx_cpu)}, ax);
}

TensorImplPtr argsort_op(const TensorImplPtr& a, int axis) {
    if (!a) throw LucidError("argsort: null input");
    const Device device = a->device_;
    OpScope scope{"argsort", device, a->dtype_, a->shape_};
    if (device == Device::GPU) {
        const auto& ga = std::get<GpuStorage>(a->storage_);
        auto out = ::mlx::core::argsort(*ga.arr, axis);
        return fresh(Storage{gpu::wrap_mlx_array(std::move(out), Dtype::I32)},
                     a->shape_, Dtype::I32, device);
    }
    int ax = wrap_axis(axis, static_cast<int>(a->shape_.size()));
    Shape out_shape = a->shape_;
...
namespace {
TensorImplPtr argext_dispatch(const TensorImplPtr& a, int axis, bool keepdims,
                              bool is_min, const char* name) {
    ...
    if (device == Device::GPU) {
        ...
        return fresh(Storage{gpu::wrap_mlx_array(std::move(out), Dtype::I32)},
                     std::move(sh), Dtype::I32, device);
    }
    int ax = wrap_axis(axis, static_cast<int>(a->shape_.size()));
    Shape out_shape = a->shape_;
    if (keepdims) out_shape[ax] = 1;
    else out_shape.erase(out_shape.begin() + ax);
...
TensorImplPtr nonzero_op(const TensorImplPtr& a) {
    ...
    switch (a->dtype_) {
        case Dtype::F32:
            check_nonzero(reinterpret_cast<const float*>(cpu.ptr.get())); break;
        case Dtype::F64:
            check_nonzero(reinterpret_cast<const double*>(cpu.ptr.get())); break;
        case Dtype::I32:
            check_nonzero(reinterpret_cast<const std::int32_t*>(cpu.ptr.get())); break;
        case Dtype::I64:
            check_nonzero(reinterpret_cast<const std::int64_t*>(cpu.ptr.get())); break;
        case Dtype::Bool:
            check_nonzero(reinterpret_cast<const std::uint8_t*>(cpu.ptr.get())); break;
        default:
    ...
    Shape out_shape{static_cast<std::int64_t>(count),
...
TensorImplPtr topk_op(const TensorImplPtr& a, std::int64_t k, int axis) {
    if (!a) throw LucidError("topk: null input");
    const Dtype dt = a->dtype_;
    const Device device = a->device_;
    ...
    if (device == Device::GPU) {
        ...
        auto out = fresh(Storage{gpu::wrap_mlx_array(std::move(values), dt)},
    ...
    if (dt == Dtype::F32)
        std::tie(out_cpu, idx_cpu) =
            sort_select_cpu<float>(ca, a->shape_, out_shape, ax, dt, true);
    else if (dt == Dtype::F64)
        std::tie(out_cpu, idx_cpu) =
            sort_select_cpu<double>(ca, a->shape_, out_shape, ax, dt, true);
    else if (dt == Dtype::I32)
        std::tie(out_cpu, idx_cpu) = sort_select_cpu<std::int32_t>(
            ca, a->shape_, out_shape, ax, dt, true);
    else if (dt == Dtype::I64)
        std::tie(out_cpu, idx_cpu) = sort_select_cpu<std::int64_t>(
            ca, a->shape_, out_shape, ax, dt, true);
    else
