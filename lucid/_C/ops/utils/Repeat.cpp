...
template <typename T>
void repeat_backward_typed(const std::byte* grad, std::byte* dst,
    ...
    for (std::size_t flat = 0; flat < total; ++flat) {
        ...
        for (std::size_t d = 0; d < ndim; ++d) {
            std::size_t coord =
                rem / static_cast<std::size_t>(out_stride[d]);
...
template <typename T>
void tile_backward_typed(const std::byte* grad, std::byte* dst,
                         const Shape& input_shape, const Shape& padded_shape,
    ...
    for (std::size_t flat = 0; flat < total; ++flat) {
        ...
        for (std::size_t d = 0; d < ndim; ++d) {
            const std::size_t coord =
                rem / static_cast<std::size_t>(out_stride[d]);
            rem -= coord * static_cast<std::size_t>(out_stride[d]);
            const std::size_t in_coord =
                coord % static_cast<std::size_t>(padded_shape[d]);
...
public:
    ...
    std::vector<Storage> apply(Storage grad_out) override {
        if (device_ == Device::GPU) {
            ...
            for (std::size_t d = 0; d < out_shape_.size(); ++d) {
                if (static_cast<int>(d) == axis_) {
                    ...
                    reshape_shape.push_back(
                        static_cast<::mlx::core::ShapeElem>(repeats_));
                } else {
                    reshape_shape.push_back(
                        static_cast<::mlx::core::ShapeElem>(out_shape_[d]));
                }
            }
            auto reshaped = ::mlx::core::reshape(*g.arr, reshape_shape);
            auto summed =
                ::mlx::core::sum(reshaped, std::vector<int>{axis_ + 1},
                                 /*keepdims=*/false);
            summed = ::mlx::core::reshape(
                summed, gpu::to_mlx_shape(input_shapes_[0]));
        ...
        switch (dtype_) {
            case Dtype::F32:
                repeat_backward_typed<float>(g.ptr.get(), out.ptr.get(),
                                             input_shapes_[0], out_shape_,
                                             axis_, repeats_);
                break;
            case Dtype::F64:
                repeat_backward_typed<double>(g.ptr.get(), out.ptr.get(),
                                              input_shapes_[0], out_shape_,
                                              axis_, repeats_);
                break;
            case Dtype::I32:
                repeat_backward_typed<std::int32_t>(
                    g.ptr.get(), out.ptr.get(), input_shapes_[0], out_shape_,
                    axis_, repeats_);
                break;
            case Dtype::I64:
                repeat_backward_typed<std::int64_t>(
                    g.ptr.get(), out.ptr.get(), input_shapes_[0], out_shape_,
                    axis_, repeats_);
                break;
            default:
                throw NotImplementedError(
                    "repeat backward: dtype not supported");
        }
        return {Storage{std::move(out)}};
    }
};

const OpSchema RepeatBackward::schema_v1{
    "repeat", 1, AmpPolicy::KeepInput, true};
    ...
    std::vector<Storage> apply(Storage grad_out) override {
        if (device_ == Device::GPU) {
            ...
            for (std::size_t d = 0; d < reps_.size(); ++d) {
                ...
                reshape_shape.push_back(
                    static_cast<::mlx::core::ShapeElem>(reps_[d]));
                reshape_shape.push_back(
                    static_cast<::mlx::core::ShapeElem>(padded_shape_[d]));
            }
            auto reshaped = ::mlx::core::reshape(*g.arr, reshape_shape);
            auto summed = sum_axes.empty()
                              ? reshaped
        ...
        switch (dtype_) {
            case Dtype::F32:
                tile_backward_typed<float>(g.ptr.get(), out.ptr.get(),
                                           input_shapes_[0], padded_shape_,
                                           out_shape_);
                break;
            case Dtype::F64:
                tile_backward_typed<double>(g.ptr.get(), out.ptr.get(),
                                            input_shapes_[0], padded_shape_,
                                            out_shape_);
                break;
            case Dtype::I32:
                tile_backward_typed<std::int32_t>(
                    g.ptr.get(), out.ptr.get(), input_shapes_[0],
                    padded_shape_, out_shape_);
                break;
            case Dtype::I64:
                tile_backward_typed<std::int64_t>(
                    g.ptr.get(), out.ptr.get(), input_shapes_[0],
                    padded_shape_, out_shape_);
                break;
            default:
...
TensorImplPtr attach_repeat_grad(const TensorImplPtr& a, TensorImplPtr out,
                                 int axis, std::int64_t repeats) {
...
TensorImplPtr attach_tile_grad(const TensorImplPtr& a, TensorImplPtr out,
                               Shape padded_shape,
...
TensorImplPtr tile_op(const TensorImplPtr& a, std::vector<std::int64_t> reps) {
    if (!a) throw LucidError("tile: null input");
    const Dtype dt = a->dtype_;
    const Device device = a->device_;
    ...
    if (device == Device::GPU) {
        ...
        auto result = fresh(Storage{gpu::wrap_mlx_array(std::move(out), dt)},
                            std::move(sh), dt, device);
        return attach_tile_grad(a, std::move(result), std::move(padded),
                                std::move(reps));
    }
    Shape padded(nout, 1);
    const std::size_t lead = nout - a->shape_.size();
    ...
    for (std::size_t d = 0; d < nout; ++d) out_shape[d] = padded[d] * reps[d];
