...
Shape resolve_reshape_shape(const Shape& in_shape,
    ...
    if (wildcard_pos != -1) {
        ...
        resolved[wildcard_pos] =
            static_cast<std::int64_t>(in_numel) / known_product;
    }
    if (shape_numel(resolved) != in_numel) {
        throw ShapeMismatch(in_shape, resolved,
                            "reshape: total numel mismatch");
    }
    return resolved;
}

TensorImplPtr build_view_output(const TensorImplPtr& a, Shape out_shape,
                                const char* op_name) {
    if (!a) throw LucidError(std::string(op_name) + ": null input");
    ...
    OpScope scope{op_name, a->device_, a->dtype_, out_shape};
    auto out_cpu = clone_for_view(std::get<CpuStorage>(a->storage_),
                                  out_shape, a->dtype_);
...
std::vector<Storage> ViewBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto out = ::mlx::core::reshape(*gg.arr,
...
// ---------------- reshape ----------------
TensorImplPtr reshape_op(const TensorImplPtr& a,
                         const std::vector<std::int64_t>& new_shape) {
    if (!a) throw LucidError("reshape: null input");
...
TensorImplPtr squeeze_op(const TensorImplPtr& a, int dim) {
    if (!a) throw LucidError("squeeze: null input");
    ...
    for (int i = 0; i < ndim; ++i) {
        if (i != wrapped) new_shape.push_back(a->shape_[static_cast<std::size_t>(i)]);
    }
...
TensorImplPtr unsqueeze_op(const TensorImplPtr& a, int dim) {
    if (!a) throw LucidError("unsqueeze: null input");
    const int ndim = static_cast<int>(a->shape_.size());
