...
CpuStorage meshgrid_backward_cpu(const CpuStorage& grad,
    ...
    for (std::size_t flat = 0; flat < total; ++flat) {
        ...
        for (std::ptrdiff_t d =
                 static_cast<std::ptrdiff_t>(output_shape.size()) - 1;
             d >= 0; --d) {
...
Storage meshgrid_backward_storage(const Storage& grad, const Shape& input_shape,
                                  const Shape& output_shape, int carry_axis,
    ...
    if (device == Device::GPU) {
        ...
        for (int d = 0; d < static_cast<int>(output_shape.size()); ++d) {
            if (d != carry_axis) axes.push_back(d);
        }
        if (axes.empty())
    ...
    switch (dt) {
        case Dtype::F32:
            return Storage{
                meshgrid_backward_cpu<float>(g, output_shape, carry_axis, dt)};
        case Dtype::F64:
            return Storage{
                meshgrid_backward_cpu<double>(g, output_shape, carry_axis, dt)};
        default:
...
public:
    ...
    std::vector<Storage> apply(Storage grad_out) override {
        return {meshgrid_backward_storage(grad_out, input_shapes_[0], out_shape_,
                                          carry_axis_, dtype_, device_)};
    }
};

const OpSchema MeshgridBackward::schema_v1{
    "meshgrid", 1, AmpPolicy::KeepInput, true};

TensorImplPtr attach_meshgrid_grad(const TensorImplPtr& input,
                                   TensorImplPtr output, int carry_axis) {
    if (!GradMode::is_enabled() || !input->requires_grad_) return output;
    ...
    bwd->set_next_edges(
        std::vector<Edge>{Edge(detail::ensure_grad_fn(input), 0)});
...
meshgrid_op(const std::vector<TensorImplPtr>& xs, bool indexing_xy) {
    ...
    if (device == Device::GPU) {
        ...
        auto out = ::mlx::core::meshgrid(std::move(arrays), false,
                                         indexing_xy ? "xy" : "ij");
        ...
        for (std::size_t i = 0; i < out.size(); ++i) {
            ...
            auto t = fresh(Storage{gpu::wrap_mlx_array(std::move(ac), dt)},
                           std::move(sh), dt, device);
            std::size_t carry_axis = i;
            if (indexing_xy && out.size() >= 2 && i < 2) carry_axis = 1 - i;
            result.push_back(attach_meshgrid_grad(
                xs[i], std::move(t), static_cast<int>(carry_axis)));
        }
        return result;
    }
    ...
    if (indexing_xy && N >= 2) std::swap(out_dims[0], out_dims[1]);
    ...
    for (std::size_t i = 0; i < N; ++i) {
        ...
        result.push_back(attach_meshgrid_grad(
            xs[i], std::move(out), static_cast<int>(carry_axis)));
    }
    return result;
}
