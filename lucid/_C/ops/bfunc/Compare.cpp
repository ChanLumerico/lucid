...
CpuStorage cmp_kernel(const CpuStorage& a, const CpuStorage& b,
                      std::size_t numel, Cmp cmp) {
    CpuStorage out;
    ...
    switch (a.dtype) {
        case Dtype::F32:
            run(reinterpret_cast<const float*>(a.ptr.get()),
                reinterpret_cast<const float*>(b.ptr.get())); break;
        case Dtype::F64:
            run(reinterpret_cast<const double*>(a.ptr.get()),
                reinterpret_cast<const double*>(b.ptr.get())); break;
        case Dtype::I32:
...
TensorImplPtr cmp_dispatch(const TensorImplPtr& a, const TensorImplPtr& b,
                           const char* name, MlxFn mlx_fn, Cmp cmp) {
    ...
    if (a->device_ == Device::GPU) {
        ...
        return fresh(Storage{gpu::wrap_mlx_array(std::move(out), Dtype::Bool)},
                     a->shape_, Dtype::Bool, a->device_);
    }
    auto out = cmp_kernel(std::get<CpuStorage>(a->storage_),
...
TensorImplPtr not_equal_op(const TensorImplPtr& a, const TensorImplPtr& b) {
    return cmp_dispatch(a, b, "not_equal",
        [](const ::mlx::core::array& x, const ::mlx::core::array& y) {
...
TensorImplPtr greater_op(const TensorImplPtr& a, const TensorImplPtr& b) {
    return cmp_dispatch(a, b, "greater",
        [](const ::mlx::core::array& x, const ::mlx::core::array& y) {
...
TensorImplPtr greater_equal_op(const TensorImplPtr& a, const TensorImplPtr& b) {
    return cmp_dispatch(a, b, "greater_equal",
        [](const ::mlx::core::array& x, const ::mlx::core::array& y) {
...
TensorImplPtr less_equal_op(const TensorImplPtr& a, const TensorImplPtr& b) {
    return cmp_dispatch(a, b, "less_equal",
        [](const ::mlx::core::array& x, const ::mlx::core::array& y) {
