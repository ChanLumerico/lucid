...
const OpSchema MaximumBackward::schema_v1{
    "maximum", /*version=*/1, AmpPolicy::Promote, /*deterministic=*/true};

CpuStorage MaximumBackward::cpu_kernel(const CpuStorage& a, const CpuStorage& b,
...
std::pair<Storage, Storage> MaximumBackward::grad_formula(const Storage& grad_out) {
    ...
    Storage mask_a = ge_mask_storage(saved_inputs_[0], saved_inputs_[1], n,
                                     dtype_, device_);  // a >= b (ties to a)
    Storage mask_b = lt_mask_storage(saved_inputs_[0], saved_inputs_[1], n,
                                     dtype_, device_);  // a < b
