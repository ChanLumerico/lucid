...
CpuStorage SubBackward::cpu_kernel(const CpuStorage& a,
                                   const CpuStorage& b,
    ...
    switch (dt) {
        case Dtype::F32:
            backend::cpu::vsub_f32(reinterpret_cast<const float*>(a.ptr.get()),
                                   reinterpret_cast<const float*>(b.ptr.get()),
                                   reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            backend::cpu::vsub_f64(reinterpret_cast<const double*>(a.ptr.get()),
                                   reinterpret_cast<const double*>(b.ptr.get()),
                                   reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
        default:
            ErrorBuilder("sub").not_implemented("dtype not supported");
    }
    return out;
}
...
GpuStorage SubBackward::gpu_kernel(const GpuStorage& a,
                                   ...
                                   Dtype dt) {
    if (!a.arr || !b.arr) {
        ErrorBuilder("sub").fail("null GPU input");
    }
