...
TensorImplPtr align_to_labels(const TensorImplPtr& t,
    ...
    for (const auto& c : target_labels) {
        if (std::find(src_labels.begin(), src_labels.end(), c) ==
            src_labels.end()) {
    ...
    for (std::size_t i = 0; i < target_labels.size(); ++i) {
        auto it = std::find(src_labels.begin(), src_labels.end(),
                            target_labels[i]);
    ...
    Shape target_shape;
    for (const auto& c : target_labels) target_shape.push_back(sizes.at(c));
...
TensorImplPtr einsum_op(const std::string& pattern,
                        const std::vector<TensorImplPtr>& operands) {
    if (operands.empty()) throw LucidError("einsum: at least one operand required");
    if (pattern.find("...") != std::string::npos)
        throw NotImplementedError(
            "einsum: ellipsis (...) patterns not yet supported in C++ engine");

    OpScope scope{"einsum", operands[0]->device_,
                  operands[0]->dtype_, operands[0]->shape_};
    ...
    if (arrow != std::string::npos) {
        ...
        for (auto& [c, v] : counts) if (v == 1) singles.push_back(c);
    ...
    for (std::size_t k = 0; k < operands.size(); ++k) {
        ...
        if (spec.size() != t->shape_.size())
            throw LucidError("einsum: operand " + std::to_string(k) +
                             " has " + std::to_string(t->shape_.size()) +
                             " axes but spec '" + spec + "' has " +
                             std::to_string(spec.size()) + " labels");
        for (std::size_t i = 0; i < spec.size(); ++i) {
            std::string c(1, spec[i]);
            std::int64_t n = t->shape_[i];
            auto it = sizes.find(c);
            if (it == sizes.end()) sizes[c] = n;
            else if (it->second != n)
                throw LucidError("einsum: label '" + c +
    ...
    if (operands.size() == 1) {
        ...
        for (char c : in_specs[0]) cur_labels.push_back(std::string(1, c));
        auto cur = operands[0];

        std::set<std::string> rhs_set;
        for (char c : rhs) rhs_set.insert(std::string(1, c));
        ...
        for (char c : rhs) rhs_labels.push_back(std::string(1, c));
        if (cur_labels != rhs_labels) {
            std::vector<int> perm(rhs_labels.size());
            for (std::size_t i = 0; i < rhs_labels.size(); ++i) {
                auto it = std::find(cur_labels.begin(), cur_labels.end(),
                                    rhs_labels[i]);
    ...
    for (std::size_t k = 1; k < operands.size(); ++k) {
        ...
        for (char c : in_specs[k]) right_labels.push_back(std::string(1, c));

        // Future labels: anything still needed (rhs OR appearing in any
        // later operand).
        std::set<std::string> future_labels;
        for (char c : rhs) future_labels.insert(std::string(1, c));
        for (std::size_t j = k + 1; j < operands.size(); ++j)
            for (char c : in_specs[j]) future_labels.insert(std::string(1, c));
        ...
        for (auto& c : union_set) {
            ...
            if (future_labels.count(c)) keep.push_back(c);
            else if (inter)             contract.push_back(c);
        ...
        auto drop_loners = [&](TensorImplPtr t,
                               std::vector<std::string>& labels,
                               const std::set<std::string>& other_set) {
            std::vector<int> kill;
            std::vector<std::string> new_labels;
            for (std::size_t i = 0; i < labels.size(); ++i) {
                if (!future_labels.count(labels[i]) &&
                    !other_set.count(labels[i])) {
                    kill.push_back(static_cast<int>(i));
                } else {
                    new_labels.push_back(labels[i]);
                }
            }
            if (!kill.empty()) t = sum_op(t, kill, /*keepdims=*/false);
