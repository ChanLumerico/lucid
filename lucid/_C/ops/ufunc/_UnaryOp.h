...
template <class T>
concept HasUnaryGpuKernel =
    requires(GpuStorage a, Shape s, Dtype d) {
...
template <class Derived>
std::shared_ptr<TensorImpl> UnaryOp<Derived>::forward(
    ...
    if (a->device_ == Device::GPU) {
        if constexpr (detail::HasUnaryGpuKernel<Derived>) {
            out_storage = Storage{Derived::gpu_kernel(
                std::get<GpuStorage>(a->storage_), a->shape_, a->dtype_)};
        } else {
            throw NotImplementedError(
                std::string(Derived::schema_v1.name) +
...
std::vector<Storage> UnaryOp<Derived>::apply(Storage grad_out) {
    ...
    return {reduce_grad_to_shape(dx, this->out_shape_, this->input_shapes_[0],
