...
CpuStorage dispatch_unary(const CpuStorage& a, const Shape& out_shape, Dtype dt,
                          FloatF32 f32, FloatF64 f64, const char* op_name) {
    ...
    switch (dt) {
        ...
        default:
            throw NotImplementedError(std::string(op_name) +
                                      ": dtype not supported in Phase 3.2");
    }
    return out;
}
...
CpuStorage NegBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    return dispatch_unary(a, out_shape, dt,
                          backend::cpu::vneg_f32,
                          backend::cpu::vneg_f64,
                          "neg");
}
...
TensorImplPtr neg_op(const TensorImplPtr& a) { return NegBackward::forward(a); }
...
CpuStorage AbsBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    return dispatch_unary(a, out_shape, dt,
                          backend::cpu::vabs_f32,
                          backend::cpu::vabs_f64,
                          "abs");
}
...
TensorImplPtr abs_op(const TensorImplPtr& a) { return AbsBackward::forward(a); }
...
CpuStorage SignBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    ...
    switch (dt) {
        case Dtype::F32: {
            ...
            for (std::size_t i = 0; i < n; ++i) q[i] = (p[i] > 0.f) - (p[i] < 0.f);
            break;
        }
        case Dtype::F64: {
            auto* p = reinterpret_cast<const double*>(a.ptr.get());
            auto* q = reinterpret_cast<double*>(out.ptr.get());
            for (std::size_t i = 0; i < n; ++i) q[i] = (p[i] > 0.0) - (p[i] < 0.0);
            break;
        }
        default:
...
TensorImplPtr sign_op(const TensorImplPtr& a) { return SignBackward::forward(a); }
...
CpuStorage ReciprocalBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    ...
    switch (dt) {
        case Dtype::F32:
            backend::cpu::vrec_f32(
                reinterpret_cast<const float*>(a.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            backend::cpu::vrec_f64(
...
TensorImplPtr reciprocal_op(const TensorImplPtr& a) { return ReciprocalBackward::forward(a); }
...
TensorImplPtr square_op(const TensorImplPtr& a) { return SquareBackward::forward(a); }
...
TensorImplPtr cube_op(const TensorImplPtr& a) { return CubeBackward::forward(a); }
