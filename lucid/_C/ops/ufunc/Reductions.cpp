...
AxisResult reduce_one_axis(const CpuStorage& in, const Shape& in_shape,
                           int axis, Dtype dt, Kernel32 k32, Kernel64 k64,
                           const char* op_name) {
    ...
    switch (dt) {
        case Dtype::F32:
            k32(reinterpret_cast<const float*>(in.ptr.get()),
                reinterpret_cast<float*>(r.data.ptr.get()),
                oir.outer, oir.reduce_dim, oir.inner);
            break;
        case Dtype::F64:
            k64(reinterpret_cast<const double*>(in.ptr.get()),
                reinterpret_cast<double*>(r.data.ptr.get()),
                oir.outer, oir.reduce_dim, oir.inner);
            break;
        default:
...
template <class Kernel32, class Kernel64>
CpuStorage multi_axis_reduce(const CpuStorage& a, const Shape& in_shape,
                             const std::vector<int>& axes, bool keepdims,
                             Dtype dt, Kernel32 k32, Kernel64 k64,
                             const char* op_name) {
...
CpuStorage SumBackward::cpu_kernel(const CpuStorage& a, const Shape& in_shape,
                                   const std::vector<int>& axes,
                                   bool keepdims, Dtype dt) {
    return multi_axis_reduce(a, in_shape, axes, keepdims, dt,
                             backend::cpu::sum_axis_f32,
                             backend::cpu::sum_axis_f64, "sum");
}

Storage SumBackward::grad_formula(const Storage& grad_out) {
    return broadcast_back_for_reduce(grad_out, this->out_shape_,
                                     this->full_input_shape_,
                                     this->reduce_axes_, this->keepdims_,
                                     this->dtype_, this->device_);
}

TensorImplPtr sum_op(const TensorImplPtr& a, const std::vector<int>& axes,
                     bool keepdims) {
...
double reduced_count(const Shape& in_shape, const std::vector<int>& axes) {
    double n = 1.0;
    for (int ax : axes) n *= static_cast<double>(in_shape[ax]);
    return n;
}
}  // namespace

CpuStorage MeanBackward::cpu_kernel(const CpuStorage& a, const Shape& in_shape,
                                    const std::vector<int>& axes,
                                    bool keepdims, Dtype dt) {
    auto out = multi_axis_reduce(a, in_shape, axes, keepdims, dt,
                                 backend::cpu::sum_axis_f32,
    ...
    switch (dt) {
        case Dtype::F32: {
            ...
            for (std::size_t i = 0; i < numel; ++i) p[i] *= inv_f;
            break;
        }
        case Dtype::F64: {
            auto* p = reinterpret_cast<double*>(out.ptr.get());
            for (std::size_t i = 0; i < numel; ++i) p[i] *= inv;
            break;
        }
        default:
...
Storage MeanBackward::grad_formula(const Storage& grad_out) {
    Storage broadcasted = broadcast_back_for_reduce(
        grad_out, this->out_shape_, this->full_input_shape_,
    ...
    return mul_scalar_storage(broadcasted, 1.0 / n,
                              shape_numel(this->full_input_shape_),
                              this->dtype_, this->device_);
}

TensorImplPtr mean_op(const TensorImplPtr& a, const std::vector<int>& axes,
                      bool keepdims) {
...
Storage ProdBackward::grad_formula(const Storage& grad_out) {
    ...
    Storage g_bcast = broadcast_back_for_reduce(
        grad_out, this->out_shape_, this->full_input_shape_,
        this->reduce_axes_, this->keepdims_, this->dtype_, this->device_);
    Storage out_bcast = broadcast_back_for_reduce(
        this->saved_output_, this->out_shape_, this->full_input_shape_,
        this->reduce_axes_, this->keepdims_, this->dtype_, this->device_);
    Storage ratio = divide_storages(out_bcast, this->saved_inputs_[0],
                                    in_numel, this->dtype_, this->device_);
    return multiply_storages(g_bcast, ratio, in_numel, this->dtype_, this->device_);
}

TensorImplPtr prod_op(const TensorImplPtr& a, const std::vector<int>& axes,
                      bool keepdims) {
...
CpuStorage MaxBackward::cpu_kernel(const CpuStorage& a, const Shape& in_shape,
                                   const std::vector<int>& axes,
                                   bool keepdims, Dtype dt) {
    return multi_axis_reduce(a, in_shape, axes, keepdims, dt,
                             backend::cpu::max_axis_f32,
...
Storage MaxBackward::grad_formula(const Storage& grad_out) {
    ...
    Storage mask_eq;
    {
        ...
        Storage ge_a = ge_mask_storage(this->saved_inputs_[0], out_bcast,
                                       in_numel, this->dtype_, this->device_);
        Storage ge_b = ge_mask_storage(out_bcast, this->saved_inputs_[0],
                                       in_numel, this->dtype_, this->device_);
        mask_eq = multiply_storages(ge_a, ge_b, in_numel,
                                    this->dtype_, this->device_);
    }
    ...
    Storage g_bcast = broadcast_back_for_reduce(
        grad_out, this->out_shape_, this->full_input_shape_,
        this->reduce_axes_, this->keepdims_, this->dtype_, this->device_);
    return multiply_storages(g_bcast, mask_eq, in_numel,
                             this->dtype_, this->device_);
}

TensorImplPtr max_op(const TensorImplPtr& a, const std::vector<int>& axes,
                     bool keepdims) {
...
CpuStorage MinBackward::cpu_kernel(const CpuStorage& a, const Shape& in_shape,
                                   const std::vector<int>& axes,
                                   bool keepdims, Dtype dt) {
    return multi_axis_reduce(a, in_shape, axes, keepdims, dt,
                             backend::cpu::min_axis_f32,
...
TensorImplPtr min_op(const TensorImplPtr& a, const std::vector<int>& axes,
                     bool keepdims) {
...
GpuStorage SumBackward::gpu_kernel(const GpuStorage& a, const Shape&,
                                   const std::vector<int>& axes, bool keepdims,
                                   Dtype dt) {
    return gpu_reduce_apply(a, dt, [&axes, keepdims](const auto& x) {
        return ::mlx::core::sum(x, axes, keepdims);
    }, "sum");
}

GpuStorage MeanBackward::gpu_kernel(const GpuStorage& a, const Shape&,
                                    const std::vector<int>& axes, bool keepdims,
                                    Dtype dt) {
    return gpu_reduce_apply(a, dt, [&axes, keepdims](const auto& x) {
...
GpuStorage ProdBackward::gpu_kernel(const GpuStorage& a, const Shape&,
                                    const std::vector<int>& axes, bool keepdims,
                                    Dtype dt) {
    return gpu_reduce_apply(a, dt, [&axes, keepdims](const auto& x) {
        return ::mlx::core::prod(x, axes, keepdims);
    }, "prod");
}

GpuStorage MaxBackward::gpu_kernel(const GpuStorage& a, const Shape&,
                                   const std::vector<int>& axes, bool keepdims,
                                   Dtype dt) {
    return gpu_reduce_apply(a, dt, [&axes, keepdims](const auto& x) {
...
GpuStorage MinBackward::gpu_kernel(const GpuStorage& a, const Shape&,
                                   const std::vector<int>& axes, bool keepdims,
                                   Dtype dt) {
    return gpu_reduce_apply(a, dt, [&axes, keepdims](const auto& x) {
