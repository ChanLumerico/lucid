...
CpuStorage RoundBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    return dispatch_float(a, out_shape, dt, backend::cpu::vround_f32, backend::cpu::vround_f64,
                          "round");
}
...
CpuStorage FloorBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    return dispatch_float(a, out_shape, dt, backend::cpu::vfloor_f32, backend::cpu::vfloor_f64,
                          "floor");
}
...
CpuStorage CeilBackward::cpu_kernel(const CpuStorage& a, const Shape& out_shape, Dtype dt) {
    return dispatch_float(a, out_shape, dt, backend::cpu::vceil_f32, backend::cpu::vceil_f64,
                          "ceil");
}
    ...
    switch (dt) {
        case Dtype::I8: {
            ...
            for (std::size_t i = 0; i < numel; ++i)
                q[i] = static_cast<std::int8_t>(~p[i]);
            break;
        }
        case Dtype::I16: {
            ...
            for (std::size_t i = 0; i < numel; ++i)
                q[i] = static_cast<std::int16_t>(~p[i]);
            break;
        }
        case Dtype::I32: {
