...
TensorImplPtr lu_solve_op(const TensorImplPtr& LU,
    ...
    auto result = backend::Dispatcher::for_device(LU->device()).linalg_lu_solve(
        LU->storage(), pivots->storage(), b->storage(),
        LU->shape(), b->shape(), LU->dtype());
