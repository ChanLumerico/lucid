...
void Adamax::update_one(std::size_t i, std::shared_ptr<TensorImpl>& p,
    ...
    if (p->device_ == Device::GPU) {
        ...
        auto new_u = ::mlx::core::maximum(
            ::mlx::core::multiply(mlx_scalar(beta2_, dt), *ug.arr),
            ::mlx::core::abs(g));
        ...
        auto update = ::mlx::core::multiply(
            step_size, ::mlx::core::divide(new_m, denom));
    ...
    else throw NotImplementedError("Adamax: dtype not supported");
}
...
Adagrad::Adagrad(std::vector<std::shared_ptr<TensorImpl>> p,
                 double lr, double eps, double wd, double init_acc)
    : Optimizer(std::move(p)),
      lr_(lr), eps_(eps), weight_decay_(wd),
      initial_accumulator_value_(init_acc) {}

void Adagrad::init_state_slot(std::size_t i,
                              const std::shared_ptr<TensorImpl>& p) {
    ...
    if (initial_accumulator_value_ != 0.0) {
        if (p->device_ == Device::GPU) {
            auto ones_arr = ::mlx::core::ones(gpu::to_mlx_shape(p->shape_),
                                              gpu::to_mlx_dtype(p->dtype_));
            auto init = ::mlx::core::multiply(
                mlx_scalar(initial_accumulator_value_, p->dtype_), ones_arr);
            ...
            if (p->dtype_ == Dtype::F32) {
                ...
                for (std::size_t k = 0; k < n; ++k) q[k] = v;
...
void Adagrad::update_one(std::size_t i, std::shared_ptr<TensorImpl>& p,
                         const Storage& grad) {
    const auto dt = p->dtype_;
    if (p->device_ == Device::GPU) {
        auto& pg = gpu_get(p->storage_);
        ...
        auto denom = ::mlx::core::add(::mlx::core::sqrt(new_ss),
                                      mlx_scalar(eps_, dt));
        auto update = ::mlx::core::multiply(mlx_scalar(lr_, dt),
    ...
    if (dt == Dtype::F32) step_cpu(cpu_ptr<float>(p->storage_),
                                    cpu_cptr<float>(grad));
                                          ...
                                          cpu_cptr<double>(grad));
    else throw NotImplementedError("Adagrad: dtype not supported");
}
...
Adadelta::Adadelta(std::vector<std::shared_ptr<TensorImpl>> p,
                   double lr, double rho, double eps, double wd)
    : Optimizer(std::move(p)),
      lr_(lr), rho_(rho), eps_(eps), weight_decay_(wd) {}

void Adadelta::init_state_slot(std::size_t i,
                               const std::shared_ptr<TensorImpl>& p) {
    if (sq_avg_.size() < params_.size()) sq_avg_.resize(params_.size());
    ...
    accumulated_update_[i] =
        make_zero_storage(p->shape_, p->dtype_, p->device_);
}

void Adadelta::update_one(std::size_t i, std::shared_ptr<TensorImpl>& p,
                          const Storage& grad) {
    const auto dt = p->dtype_;
    if (p->device_ == Device::GPU) {
        auto& pg = gpu_get(p->storage_);
        ...
        auto new_sq = ::mlx::core::add(
            ...
            ::mlx::core::multiply(mlx_scalar(1.0 - rho_, dt),
                                  ::mlx::core::square(g)));
        gpu_replace(sq, ::mlx::core::array(new_sq), dt);
        auto update = ::mlx::core::multiply(
            ::mlx::core::divide(
                ::mlx::core::sqrt(::mlx::core::add(*acc.arr,
                                                   mlx_scalar(eps_, dt))),
                ::mlx::core::sqrt(::mlx::core::add(new_sq,
                                                   mlx_scalar(eps_, dt)))),
            g);
        auto new_acc = ::mlx::core::add(
            ::mlx::core::multiply(mlx_scalar(rho_, dt), *acc.arr),
            ::mlx::core::multiply(mlx_scalar(1.0 - rho_, dt),
                                  ::mlx::core::square(update)));
        gpu_replace(acc, ::mlx::core::array(new_acc), dt);
        gpu_replace(pg, ::mlx::core::subtract(
            *pg.arr, ::mlx::core::multiply(mlx_scalar(lr_, dt), update)), dt);
        return;
    }
    const std::size_t n = cpu_numel(*p);
    auto step_cpu = [&](auto* P, const auto* G) {
        ...
        for (std::size_t k = 0; k < n; ++k) {
            ...
            const T update = (std::sqrt(AC[k] + epsT) /
                              std::sqrt(SQ[k] + epsT)) * g;
    ...
    if (dt == Dtype::F32) step_cpu(cpu_ptr<float>(p->storage_),
                                    cpu_cptr<float>(grad));
                                          ...
                                          cpu_cptr<double>(grad));
    else throw NotImplementedError("Adadelta: dtype not supported");
}
