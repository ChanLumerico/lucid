...
LRScheduler::LRScheduler(Optimizer& opt)
    : opt_(opt), base_lr_(opt.lr()), epoch_(0) {}
...
ExponentialLR::ExponentialLR(Optimizer& opt, double gamma)
...
MultiStepLR::MultiStepLR(Optimizer& opt, std::vector<std::int64_t> milestones,
                         double gamma)
...
CosineAnnealingLR::CosineAnnealingLR(Optimizer& opt, std::int64_t T_max,
                                     double eta_min)
...
double CosineAnnealingLR::compute_lr_at(std::int64_t epoch) const {
    ...
    return eta_min_ + 0.5 * (base_lr_ - eta_min_) *
           (1.0 + std::cos(PI * static_cast<double>(epoch) /
                           static_cast<double>(T_max_)));
}

// ---------------- LambdaLR ----------------

LambdaLR::LambdaLR(Optimizer& opt,
...
ReduceLROnPlateau::ReduceLROnPlateau(Optimizer& opt, Mode mode,
                                     double factor, std::int64_t patience,
                                     double threshold,
                                     ThresholdMode threshold_mode,
                                     std::int64_t cooldown, double min_lr,
                                     double eps)
    : opt_(opt), mode_(mode), factor_(factor), patience_(patience),
      threshold_(threshold), threshold_mode_(threshold_mode),
      cooldown_(cooldown), min_lr_(min_lr), eps_(eps),
      best_(mode == Mode::Min ?  std::numeric_limits<double>::infinity()
                              : -std::numeric_limits<double>::infinity()),
      num_bad_epochs_(0), cooldown_counter_(0), last_lr_(opt.lr()) {
    if (factor_ >= 1.0)
...
bool ReduceLROnPlateau::is_better(double metric) const {
    ...
    if (threshold_mode_ == ThresholdMode::Rel) {
        thresh = (mode_ == Mode::Min) ? best_ * (1.0 - threshold_)
                                      : best_ * (1.0 + threshold_);
    } else {
        thresh = (mode_ == Mode::Min) ? best_ - threshold_
                                      : best_ + threshold_;
    }
...
CyclicLR::CyclicLR(Optimizer& opt, double base_lr, double max_lr,
                   std::int64_t step_size_up, std::int64_t step_size_down,
                   Mode mode, double gamma)
    : LRScheduler(opt),
      base_lr_cyc_(base_lr), max_lr_(max_lr),
...
double CyclicLR::compute_lr_at(std::int64_t epoch) const {
    ...
    const double x = std::abs(static_cast<double>(epoch) /
                              static_cast<double>(step_size_up_) -
                              2.0 * cycle - 1.0);
    double scale = 1.0;
    switch (mode_) {
        case Mode::Triangular:  scale = 1.0; break;
        case Mode::Triangular2: scale = 1.0 / static_cast<double>(1ll << cycle); break;
        case Mode::ExpRange:    scale = std::pow(gamma_, static_cast<double>(epoch)); break;
    }
    return base_lr_cyc_ + (max_lr_ - base_lr_cyc_) *
           std::max(0.0, 1.0 - x) * scale;
}

// ---------------- NoamScheduler ----------------

NoamScheduler::NoamScheduler(Optimizer& opt, std::int64_t model_size,
                             std::int64_t warmup_steps, double factor)
    : LRScheduler(opt), model_size_(model_size),
      warmup_steps_(warmup_steps), factor_(factor) {
    if (model_size <= 0) throw LucidError("NoamScheduler: model_size must be > 0");
    if (warmup_steps <= 0) throw LucidError("NoamScheduler: warmup_steps must be > 0");
    if (factor <= 0) throw LucidError("NoamScheduler: factor must be > 0");
}

double NoamScheduler::compute_lr_at(std::int64_t epoch) const {
    const double step = std::max<std::int64_t>(epoch, 1);
    const double scale = factor_ *
        std::pow(static_cast<double>(model_size_), -0.5);
    const double warmup_term = step *
        std::pow(static_cast<double>(warmup_steps_), -1.5);
