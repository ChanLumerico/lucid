...
const OpSchema EmbeddingBackward::schema_v1{
    "embedding", 1, AmpPolicy::Promote, true};

TensorImplPtr EmbeddingBackward::forward(const TensorImplPtr& weight,
                                           const TensorImplPtr& indices,
                                           int padding_idx) {
    if (!weight || !indices) throw LucidError("embedding: null input");
    ...
    if (weight->device_ == Device::GPU) {
        ...
        if (padding_idx >= 0) {
            ...
            auto pad = ::mlx::core::astype(::mlx::core::array(padding_idx),
                                              ::mlx::core::int64);
        ...
        for (std::size_t i = 0; i < M; ++i) {
            ...
            if (padding_idx >= 0 && id == padding_idx) {
                ...
                const std::byte* src = ws.ptr.get() +
                    static_cast<std::size_t>(id) * row_bytes;
    ...
    auto out = std::make_shared<TensorImpl>(std::move(out_storage),
                                             out_shape, weight->dtype_,
                                             weight->device_, false);

    if (!GradMode::is_enabled() || !weight->requires_grad_) return out;
...
std::vector<Storage> EmbeddingBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        if (padding_idx_ >= 0) {
            ...
            auto mask_b = ::mlx::core::reshape(mask_dt,
                {static_cast<int>(M), 1});
        ...
        auto idx_col = ::mlx::core::reshape(idx_flat,
            {static_cast<int>(M), 1});
...
TensorImplPtr sinusoidal_pos_embedding_op(std::int64_t seq_len,
    ...
    if (seq_len < 0) throw LucidError("sinusoidal_pos_embedding: seq_len < 0");
    if (embed_dim <= 0)
    ...
    if (device == Device::GPU) {
        if (L * D == 0) {
            auto z = ::mlx::core::zeros(gpu::to_mlx_shape(out_shape),
                                          gpu::to_mlx_dtype(dtype));
            return std::make_shared<TensorImpl>(
                Storage{gpu::wrap_mlx_array(std::move(z), dtype)},
        ...
        auto pos = ::mlx::core::reshape(
            ::mlx::core::astype(::mlx::core::arange(0, static_cast<int>(L), 1),
                                  mlx_dt),
            {static_cast<int>(L), 1});
        auto k = ::mlx::core::astype(::mlx::core::arange(0, static_cast<int>(Dh), 1),
                                        mlx_dt);
        const double inv_d = -std::log(10000.0) / static_cast<double>(D);
        auto two_inv_d = ::mlx::core::astype(::mlx::core::array(static_cast<float>(2.0 * inv_d)), mlx_dt);
        ...
        auto stacked = ::mlx::core::concatenate(
            std::vector<::mlx::core::array>{sin_e, cos_e}, /*axis=*/-1);  // [L, Dh, 2]
        auto out = ::mlx::core::reshape(stacked,
            {static_cast<int>(L), static_cast<int>(2 * Dh)});  // [L, 2*Dh]
        if (D % 2 != 0) {
            // Append a zero column.
            auto pad = ::mlx::core::zeros({static_cast<int>(L), 1}, mlx_dt);
            out = ::mlx::core::concatenate(
                std::vector<::mlx::core::array>{out, pad}, -1);
        }
        return std::make_shared<TensorImpl>(
            Storage{gpu::wrap_mlx_array(std::move(out), dtype)},
            out_shape, dtype, device, false);
    }
    ...
    if (dtype == Dtype::F32) run(float{});
    else if (dtype == Dtype::F64) run(double{});
    else throw NotImplementedError(
        "sinusoidal_pos_embedding: dtype must be F32 or F64");
...
const OpSchema RotaryPosEmbeddingBackward::schema_v1{
    "rotary_pos_embedding", 1, AmpPolicy::ForceFP32, true};
...
template <typename T>
void build_cos_sin_tables(const double* pos, const double* theta,
                          std::size_t L, std::size_t Dh,
                          T* cos_out, T* sin_out) {
    for (std::size_t i = 0; i < L; ++i) {
        for (std::size_t k = 0; k < Dh; ++k) {
...
template <typename T>
void rope_forward(const T* xp, const T* cosp, const T* sinp, T* op,
                  std::size_t batch, std::size_t L, std::size_t D,
                  bool interleaved) {
    const std::size_t Dh = D / 2;
...
template <typename T>
void rope_backward(const T* gp, const T* cosp, const T* sinp, T* dxp,
                   std::size_t batch, std::size_t L, std::size_t D,
                   bool interleaved) {
    const std::size_t Dh = D / 2;
    for (std::size_t b = 0; b < batch; ++b) {
...
TensorImplPtr RotaryPosEmbeddingBackward::forward(
    const TensorImplPtr& input,
    const TensorImplPtr& position_ids_or_null,
    bool interleaved) {
    if (!input) throw LucidError("rotary_pos_embedding: null input");
    ...
    if (input->shape_.size() < 2)
        throw LucidError(
            "rotary_pos_embedding: input must be at least 2-D ([..., L, D])");
    ...
    if (input->device_ == Device::GPU) {
        ...
        if (position_ids_or_null) {
            ...
            pos_arr = ::mlx::core::astype(
                ::mlx::core::arange(0, static_cast<int>(L), 1), mlx_dt);
        }
        ...
        auto k_arr = ::mlx::core::astype(
            ::mlx::core::arange(0, static_cast<int>(Dh), 1), mlx_dt);
        auto coef = ::mlx::core::astype(::mlx::core::array(
            static_cast<float>(-2.0 * base / static_cast<double>(D))), mlx_dt);
        ...
        if (interleaved) {
            ...
            auto stacked = ::mlx::core::concatenate(
                std::vector<::mlx::core::array>{out_e_e, out_o_e}, -1);
            out_arr = ::mlx::core::reshape(stacked, in_shape);
        } else {
            // Split: x[..., :Dh] and x[..., Dh:]. Use take with index ranges.
            auto idx_first = ::mlx::core::astype(
                ::mlx::core::arange(0, static_cast<int>(Dh), 1),
                ::mlx::core::int64);
            ...
            out_arr = ::mlx::core::concatenate(
                std::vector<::mlx::core::array>{out_a, out_b}, -1);
        }

        auto out = std::make_shared<TensorImpl>(
            Storage{gpu::wrap_mlx_array(std::move(out_arr), input->dtype_)},
    ...
    if (position_ids_or_null) {
        ...
        for (std::size_t i = 0; i < L; ++i) {
            switch (position_ids_or_null->dtype_) {
                case Dtype::I8:  pos[i] = reinterpret_cast<const std::int8_t*>(ps.ptr.get())[i]; break;
                case Dtype::I16: pos[i] = reinterpret_cast<const std::int16_t*>(ps.ptr.get())[i]; break;
                case Dtype::I32: pos[i] = reinterpret_cast<const std::int32_t*>(ps.ptr.get())[i]; break;
                case Dtype::I64: pos[i] = reinterpret_cast<const std::int64_t*>(ps.ptr.get())[i]; break;
                case Dtype::F32: pos[i] = reinterpret_cast<const float*>(ps.ptr.get())[i]; break;
                case Dtype::F64: pos[i] = reinterpret_cast<const double*>(ps.ptr.get())[i]; break;
    ...
    const double base = std::log(10000.0);
    for (std::size_t k = 0; k < Dh; ++k) {
        theta[k] = std::exp(-2.0 * static_cast<double>(k) * base /
    ...
    auto run = [&](auto type_tag) {
        ...
        rope_forward<T>(reinterpret_cast<const T*>(xs.ptr.get()),
                         ...
                         reinterpret_cast<T*>(out_cpu.ptr.get()),
                         batch, L, D, interleaved);
    };
    if (input->dtype_ == Dtype::F32) run(float{});
    else if (input->dtype_ == Dtype::F64) run(double{});
    else throw NotImplementedError(
        "rotary_pos_embedding: dtype must be F32 or F64");
...
std::vector<Storage> RotaryPosEmbeddingBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        if (interleaved_) {
            ...
            auto stacked = ::mlx::core::concatenate(
                std::vector<::mlx::core::array>{dxe_e, dxo_e}, -1);
            out_arr = ::mlx::core::reshape(stacked, in_shape);
        } else {
            auto idx_first = ::mlx::core::astype(
                ::mlx::core::arange(0, static_cast<int>(Dh), 1), ::mlx::core::int64);
            auto idx_second = ::mlx::core::astype(
                ::mlx::core::arange(static_cast<int>(Dh), static_cast<int>(D), 1),
                ::mlx::core::int64);
            ...
            out_arr = ::mlx::core::concatenate(
                std::vector<::mlx::core::array>{dxa, dxb}, -1);
        }
    ...
    auto run = [&](auto type_tag) {
        ...
        rope_backward<T>(reinterpret_cast<const T*>(gs.ptr.get()),
                          ...
                          reinterpret_cast<T*>(dx.ptr.get()),
                          batch, L, D, interleaved_);
    };
    if (dtype_ == Dtype::F32) run(float{});
    else if (dtype_ == Dtype::F64) run(double{});
    else throw NotImplementedError(
        "rotary_pos_embedding backward: dtype not supported");
...
TensorImplPtr rotary_pos_embedding_op(const TensorImplPtr& input,
                                       const TensorImplPtr& position_ids_or_null,
                                       bool interleaved) {
    return RotaryPosEmbeddingBackward::forward(input, position_ids_or_null,
                                                 interleaved);
}
