...
inline ::mlx::core::array mlx_neg(const ::mlx::core::array& a) {
    return ::mlx::core::negative(a);
}
...
::mlx::core::array mlx_apply_reduction(const ::mlx::core::array& l, Reduction red) {
    if (red == Reduction::None) return l;
...
::mlx::core::array mlx_grad_scale(const ::mlx::core::array& gout, Reduction red,
                                    std::size_t numel, ::mlx::core::Dtype dt) {
    if (red == Reduction::None) return gout;
    if (red == Reduction::Mean) {
        return ::mlx::core::divide(
            gout, mlx_scalar(static_cast<double>(numel), dt));
    }
    return gout;  // Sum
}
...
CpuStorage make_scalar(double val, Dtype dt) {
    ...
    switch (dt) {
        case Dtype::F32: *reinterpret_cast<float*>(s.ptr.get())  = static_cast<float>(val); break;
        case Dtype::F64: *reinterpret_cast<double*>(s.ptr.get()) = val; break;
        default: throw NotImplementedError("loss: dtype not supported");
    }
    return s;
}
...
template <typename T>
Storage apply_reduction(const T* lp, std::size_t numel,
                        Reduction red, Dtype dt) {
    ...
    switch (ts.dtype) {
        case Dtype::I8:
            return static_cast<std::int64_t>(
                reinterpret_cast<const std::int8_t*>(ts.ptr.get())[i]);
        case Dtype::I16:
            return static_cast<std::int64_t>(
        ...
        case Dtype::F32:
            return static_cast<std::int64_t>(
                reinterpret_cast<const float*>(ts.ptr.get())[i]);
        case Dtype::F64:
            return static_cast<std::int64_t>(
                reinterpret_cast<const double*>(ts.ptr.get())[i]);
        default:
...
TensorImplPtr MseLossBackward::forward(const TensorImplPtr& input,
                                        ...
                                        Reduction reduction) {
    if (!input || !target) throw LucidError("mse_loss: null input");
    ...
    if (input->device_ == Device::GPU) {
        ...
        if (!gx.arr || !gt.arr) throw LucidError("mse_loss: null GPU array");
    ...
    if (!GradMode::is_enabled() ||
        !(input->requires_grad_ || target->requires_grad_)) return out;
...
std::vector<Storage> MseLossBackward::apply(Storage grad_out) {
    ...
    if (dtype_ == Dtype::F32) compute(float{});
    else if (dtype_ == Dtype::F64) compute(double{});
    else throw NotImplementedError("mse_loss backward: dtype not supported");
    return {Storage{std::move(dx_cpu)}, Storage{std::move(dt_cpu)}};
}

TensorImplPtr mse_loss_op(const TensorImplPtr& input, const TensorImplPtr& target,
                            int reduction) {
...
TensorImplPtr BCELossBackward::forward(const TensorImplPtr& input,
    ...
    if (!input || !target || !weight)
        throw LucidError("bce_loss: input/target/weight required (pass ones for weight if not used)");
    if (input->shape_ != target->shape_)
        throw ShapeMismatch(input->shape_, target->shape_,
    ...
    if (input->device_ == Device::GPU) {
        ...
        auto p = ::mlx::core::clip(*gx.arr,
                                   std::optional<::mlx::core::array>(e_lo),
        ...
        auto compute = [&](auto type_tag) {
            ...
            for (std::size_t i = 0; i < numel; ++i) {
                ...
                const T l = -(tp[i] * std::log(p)
                               + (static_cast<T>(1) - tp[i])
                                 * std::log(static_cast<T>(1) - p));
                lp[i] = wp[i] * l;
            }
        };
        if (input->dtype_ == Dtype::F32) compute(float{});
        else if (input->dtype_ == Dtype::F64) compute(double{});
        else throw NotImplementedError("bce_loss: dtype not supported");

        if (input->dtype_ == Dtype::F32) {
            out_storage = apply_reduction<float>(
                reinterpret_cast<float*>(loss_buf.ptr.get()),
                numel, reduction, input->dtype_);
        } else {
            out_storage = apply_reduction<double>(
                reinterpret_cast<double*>(loss_buf.ptr.get()),
                numel, reduction, input->dtype_);
        }
    }
    ...
    bwd->set_next_edges(std::vector<Edge>{
        Edge(x_edge, 0), Edge(t_edge, 0), Edge(w_edge, 0)});
...
std::vector<Storage> BCELossBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto dlp = ::mlx::core::add(
            ::mlx::core::negative(::mlx::core::divide(*gt.arr, p)),
            ::mlx::core::divide(one_mt, one_mp));
        auto dly = ::mlx::core::add(::mlx::core::negative(log_p), log_1mp);
        auto l = ::mlx::core::negative(::mlx::core::add(
            ::mlx::core::multiply(*gt.arr, log_p),
    ...
    auto compute = [&](auto type_tag) {
        ...
        for (std::size_t i = 0; i < numel; ++i) {
            ...
            dtp[i] = w * dly * scale;
            const T l = -(y * std::log(p)
                           + (static_cast<T>(1) - y) * std::log(static_cast<T>(1) - p));
            dwp[i] = l * scale;
        }
    };
    if (dtype_ == Dtype::F32) compute(float{});
    else if (dtype_ == Dtype::F64) compute(double{});
    else throw NotImplementedError("bce_loss backward: dtype not supported");
    return {Storage{std::move(dx_cpu)}, Storage{std::move(dt_cpu)},
            Storage{std::move(dw_cpu)}};
}

TensorImplPtr bce_loss_op(const TensorImplPtr& input, const TensorImplPtr& target,
                           const TensorImplPtr& weight,
                           int reduction, double eps) {
    return BCELossBackward::forward(input, target, weight,
                                     static_cast<Reduction>(reduction), eps);
}
...
const OpSchema BCEWithLogitsBackward::schema_v1{
    "bce_with_logits", 1, AmpPolicy::ForceFP32, true};
...
TensorImplPtr BCEWithLogitsBackward::forward(const TensorImplPtr& input,
    ...
    if (input->device_ == Device::GPU) {
        ...
        else throw NotImplementedError("bce_with_logits: dtype not supported");
    ...
    bwd->orig_shape_    = input->shape_;
    bwd->set_next_edges(std::vector<Edge>{
        Edge(x_edge, 0), Edge(t_edge, 0),
        Edge(w_edge, 0), Edge(pw_edge, 0)});
    bwd->set_saved_versions(std::vector<std::int64_t>{
        static_cast<std::int64_t>(input->version_),
        static_cast<std::int64_t>(target->version_),
        static_cast<std::int64_t>(weight->version_),
...
std::vector<Storage> BCEWithLogitsBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto dpw       = ::mlx::core::multiply(*gw.arr,
                            ::mlx::core::multiply(*gt.arr, log1pexp));
    ...
    auto compute = [&](auto type_tag) {
        ...
        for (std::size_t i = 0; i < numel; ++i) {
            ...
            const T log_weight = (pw - static_cast<T>(1)) * y + static_cast<T>(1);
            const T log1pexp = std::log1p(std::exp(-std::abs(x)));
            const T l = std::max(x, static_cast<T>(0)) - x * y
                         + log_weight * log1pexp;
            const T w = wp[i];
    ...
    else throw NotImplementedError("bce_with_logits backward: dtype not supported");
    return {Storage{std::move(dx_cpu)}, Storage{std::move(dt_cpu)},
            Storage{std::move(dw_cpu)}, Storage{std::move(dpw_cpu)}};
}
...
const OpSchema CrossEntropyBackward::schema_v1{
    "cross_entropy_loss", 1, AmpPolicy::ForceFP32, true};
...
TensorImplPtr CrossEntropyBackward::forward(const TensorImplPtr& input,
                                              ...
                                              Reduction reduction, double eps,
                                              int ignore_index) {
    if (!input || !target) throw LucidError("cross_entropy: null input");
    if (input->shape_.size() < 2)
        throw ShapeMismatch(input->shape_, Shape{},
                             "cross_entropy: input must be (N, C, ...)");
    ...
    if (input->device_ == Device::GPU) {
        ...
        auto safe_t  = ::mlx::core::where(ig_mask, t_idx,
                                            ::mlx::core::astype(::mlx::core::array(0),
                                                                  ::mlx::core::int64));
        ...
        auto neg_log_pred = ::mlx::core::negative(::mlx::core::log(
            ::mlx::core::add(pred, eps_arr)));
        ...
        Storage out_storage;
        Shape out_shape_local =
            (reduction == Reduction::None) ? per_sample_shape : Shape{};
        if (reduction == Reduction::None) {
            out_storage = Storage{gpu::wrap_mlx_array(std::move(loss_squeezed),
                                                        input->dtype_)};
        ...
        bwd->saved_softmax_      = Storage{gpu::wrap_mlx_array(std::move(softmax),
                                                                  input->dtype_)};
        bwd->saved_target_       = target->storage_;
        if (weight_or_null) bwd->saved_weight_ = weight_or_null->storage_;
        // Save valid_count as a 1-elem GPU array.
        bwd->saved_valid_count_  = Storage{gpu::wrap_mlx_array(std::move(vc_safe),
                                                                  input->dtype_)};
    ...
    auto softmax_buf = allocate_size(static_cast<std::size_t>(N) * C * spatial,
                                      input->dtype_);
    auto loss_buf = allocate_size(total_samples, input->dtype_);

    const auto& xs = std::get<CpuStorage>(input->storage_);
    const auto& ts = std::get<CpuStorage>(target->storage_);
    const CpuStorage* ws = weight_or_null
        ? &std::get<CpuStorage>(weight_or_null->storage_) : nullptr;
    ...
    else throw NotImplementedError("cross_entropy: dtype not supported");

    std::size_t valid_count = 0;
    for (std::size_t i = 0; i < total_samples; ++i) {
        if (read_target(ts, i) != ignore_index) ++valid_count;
    }
    ...
    if (input->dtype_ == Dtype::F32) {
        ...
        if (reduction == Reduction::None) {
            ...
            if (reduction == Reduction::Mean) s /= static_cast<float>(valid_count);
        ...
        if (reduction == Reduction::None) {
            ...
            if (reduction == Reduction::Mean) s /= static_cast<double>(valid_count);
    ...
    bwd->saved_target_  = target->storage_;
    if (weight_or_null) bwd->saved_weight_ = weight_or_null->storage_;
    bwd->saved_valid_count_ = Storage{make_scalar(
        static_cast<double>(valid_count), input->dtype_)};
    bwd->set_next_edges(std::vector<Edge>{Edge(x_edge, 0)});
    bwd->set_saved_versions(std::vector<std::int64_t>{
        static_cast<std::int64_t>(input->version_)});
    out->grad_fn_       = std::move(bwd);
...
std::vector<Storage> CrossEntropyBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto t_idx = ::mlx::core::reshape(
            ::mlx::core::astype(*tgt.arr, ::mlx::core::int64), t_shape_with_axis);
    ...
    auto compute = [&](auto type_tag) {
        ...
        auto* dxp = reinterpret_cast<T*>(dx_cpu.ptr.get());
        const T* wp = ws ? reinterpret_cast<const T*>(ws->ptr.get()) : nullptr;
        const T valid = *reinterpret_cast<const T*>(
            std::get<CpuStorage>(saved_valid_count_).ptr.get());
        const bool elem = (reduction_ == Reduction::None);

        for (int n = 0; n < N; ++n) {
            for (int s = 0; s < spatial; ++s) {
                const std::int64_t y = read_target(tgt, n * spatial + s);
                if (static_cast<int>(y) == ignore_index_) continue;
                const T go = elem ? gp[n * spatial + s] : gp[0];
                const T scale = (reduction_ == Reduction::Mean)
                                  ? (go / valid) : go;
                const T w = wp ? wp[static_cast<int>(y)] : static_cast<T>(1);
                for (int c = 0; c < C; ++c) {
                    T v = sp[(n * C + c) * spatial + s];
                    if (c == static_cast<int>(y)) v -= static_cast<T>(1);
    ...
    else throw NotImplementedError("cross_entropy backward: dtype not supported");
...
TensorImplPtr NLLLossBackward::forward(const TensorImplPtr& input,
    ...
    if (!input || !target) throw LucidError("nll_loss: null input");
    if (input->shape_.size() < 2)
        throw ShapeMismatch(input->shape_, Shape{},
                             "nll_loss: input must be (N, C, ...)");
    if (input->device_ != target->device_)
        throw DeviceMismatch(std::string(device_name(input->device_)),
                              std::string(device_name(target->device_)),
    ...
    if (input->device_ == Device::GPU) {
        ...
        t_shape_with_axis.insert(t_shape_with_axis.begin() + 1, 1);
        auto t_idx = ::mlx::core::reshape(
            ::mlx::core::astype(*gt.arr, ::mlx::core::int64), t_shape_with_axis);
        auto ig_arr = ::mlx::core::astype(::mlx::core::array(ignore_index), ::mlx::core::int64);
    ...
    else throw NotImplementedError("nll_loss: dtype not supported");
    if (valid_count == 0) valid_count = 1;
...
std::vector<Storage> NLLLossBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto dx = ::mlx::core::multiply(::mlx::core::multiply(neg_onehot, w_full),
                                          scaled_full);
        return {Storage{gpu::wrap_mlx_array(std::move(dx), dtype_)}};
    }
    ...
    else throw NotImplementedError("nll_loss backward: dtype not supported");
    return {Storage{std::move(dx_cpu)}};
}

TensorImplPtr nll_loss_op(const TensorImplPtr& input, const TensorImplPtr& target,
                            const TensorImplPtr& weight_or_null,
...
TensorImplPtr HuberLossBackward::forward(const TensorImplPtr& input,
    ...
    if (!input || !target) throw LucidError("huber_loss: null input");
    if (input->shape_ != target->shape_)
        throw ShapeMismatch(input->shape_, target->shape_,
    ...
    if (input->device_ == Device::GPU) {
        ...
        else throw NotImplementedError("huber_loss: dtype not supported");
...
std::vector<Storage> HuberLossBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto ar    = ::mlx::core::abs(r);
        auto cond  = ::mlx::core::less(ar, d);
        auto sgn_d = ::mlx::core::where(::mlx::core::greater(r, mlx_scalar(0.0, mlx_dt)),
                                          d, neg_d);
    ...
    else throw NotImplementedError("huber_loss backward: dtype not supported");
    return {Storage{std::move(dx_cpu)}, Storage{std::move(dt_cpu)}};
}

TensorImplPtr huber_loss_op(const TensorImplPtr& input, const TensorImplPtr& target,
                              double delta, int reduction) {
    return HuberLossBackward::forward(input, target, delta,
                                       static_cast<Reduction>(reduction));
}
