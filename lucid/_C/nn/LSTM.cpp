...
std::vector<Storage> LstmBackward::apply(Storage grad_out) {
    ...
    return be.lstm_backward(
        grad_out, zero_hn, zero_cn,
        saved_input, saved_h0, saved_weights,
        gates_all, cells_all, opts, dtype);
}

// ---------------------------------------------------------------------------
// LstmBackward::forward
// ---------------------------------------------------------------------------

std::vector<TensorImplPtr> LstmBackward::forward(
    ...
    for (const auto& w : weights) {
        if (!w) ErrorBuilder("lstm").fail("null weight");
    ...
    Shape out_shape{static_cast<std::int64_t>(T),
                    static_cast<std::int64_t>(B),
                    static_cast<std::int64_t>(H)};
    Shape hn_shape{static_cast<std::int64_t>(opts.num_layers),
                   static_cast<std::int64_t>(B),
                   static_cast<std::int64_t>(H)};

    if (!needs_grad) {
        auto res = be.lstm_forward(input->storage(), h0->storage(), c0->storage(),
    ...
    // Training forward: saves gates + cells for BPTT.
    auto res = be.lstm_forward_train(input->storage(), h0->storage(), c0->storage(),
                                     w_storages, opts, dt);
    if (res.size() < 5)
    ...
    // Edges: input, h0, c0, wih, whh, bih, bhh (must match lstm_backward output order)
    std::vector<TensorImplPtr> edge_tensors{input, h0, c0};
    for (const auto& w : weights) edge_tensors.push_back(w);
...
std::vector<TensorImplPtr> lstm_op(
        const TensorImplPtr& input,
        const TensorImplPtr& h0,
        const TensorImplPtr& c0,
        const std::vector<TensorImplPtr>& weights,
