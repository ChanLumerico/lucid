...
template <>
void gemm_typed<float>(bool transA,
                       bool transB,
                       int M,
                       int N,
                       int K,
                       ...
                       float* C,
                       int ldc) {
    backend::cpu::sgemm(transA, transB, M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
}

template <>
void gemm_typed<double>(bool transA,
                        bool transB,
                        int M,
                        int N,
                        int K,
                        ...
                        double* C,
                        int ldc) {
    backend::cpu::dgemm(transA, transB, M, N, K, alpha, A, lda, B, ldb, beta, C, ldc);
}
...
template <typename T>
void apply_additive_mask(T* scores,
                         const T* mask,
                         std::size_t B,
                         std::size_t Lq,
                         std::size_t Lk,
    ...
    if (mask_numel == per_batch) {
        ...
        throw ShapeMismatch(Shape{static_cast<std::int64_t>(mask_numel)},
                            Shape{static_cast<std::int64_t>(B), static_cast<std::int64_t>(Lq),
...
template <typename T>
void apply_causal_mask(T* scores, std::size_t B, std::size_t Lq, std::size_t Lk) {
    const T neg_inf = -std::numeric_limits<T>::infinity();
    for (std::size_t b = 0; b < B; ++b) {
...
template <typename T>
void softmax_backward_rows(
    const T* w, const T* dw, T* dscores, std::size_t rows, std::size_t cols) {
    for (std::size_t r = 0; r < rows; ++r) {
        const T* wr = w + r * cols;
...
template <typename T>
void compute_attention_forward(const T* Qp,
                               const T* Kp,
                               const T* Vp,
    ...
    for (std::size_t b = 0; b < ctx.B; ++b) {
        ...
        gemm_typed<T>(/*transA=*/false, /*transB=*/true, static_cast<int>(ctx.Lq),
                      static_cast<int>(ctx.Lk), static_cast<int>(ctx.Dk), ctx.scale, Qb,
                      static_cast<int>(ctx.Dk), Kb, static_cast<int>(ctx.Dk), T{0}, sb,
    ...
    for (std::size_t b = 0; b < ctx.B; ++b) {
        ...
        gemm_typed<T>(false, false, static_cast<int>(ctx.Lq), static_cast<int>(ctx.Dv),
                      static_cast<int>(ctx.Lk), T{1}, Wb, static_cast<int>(ctx.Lk), Vb,
                      static_cast<int>(ctx.Dv), T{0}, Ob, static_cast<int>(ctx.Dv));
    }
}
...
ForwardCore run_forward(const TensorImplPtr& q,
    ...
    return ForwardCore{
        std::move(out),       std::move(weights_storage), fq.B, fq.L, fk.L, fq.D, fv.D,
...
std::vector<Storage> ScaledDotProductAttentionBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto scale_arr =
            ::mlx::core::astype(::mlx::core::array(static_cast<float>(scale_)), mlx_dt);
    ...
    auto run = [&](auto type_tag) {
        ...
        for (std::size_t b = 0; b < B; ++b) {
            ...
            gemm_typed<T>(/*transA=*/true, /*transB=*/false, static_cast<int>(Lk),
                          static_cast<int>(Dv), static_cast<int>(Lq), T{1}, Wb,
                          static_cast<int>(Lk), Gb, static_cast<int>(Dv), T{0}, dVb,
                          static_cast<int>(Dv));

            // dweights[b] = dout[b] @ V[b]^T.
            //   shape: (Lq, Dv) @ (Dv, Lk) -> (Lq, Lk)
            gemm_typed<T>(false, true, static_cast<int>(Lq), static_cast<int>(Lk),
                          static_cast<int>(Dv), T{1}, Gb, static_cast<int>(Dv), Vb,
                          static_cast<int>(Dv), T{0}, dweights.data(), static_cast<int>(Lk));

            // dscores[b] = softmax_backward(weights[b], dweights[b]).
            softmax_backward_rows<T>(Wb, dweights.data(), dscores.data(), Lq, Lk);

            // dQ[b] = scale * dscores[b] @ K[b].
            //   shape: (Lq, Lk) @ (Lk, Dk) -> (Lq, Dk)
            gemm_typed<T>(false, false, static_cast<int>(Lq), static_cast<int>(Dk),
                          static_cast<int>(Lk), scale_t, dscores.data(), static_cast<int>(Lk), Kb,
                          static_cast<int>(Dk), T{0}, dQb, static_cast<int>(Dk));

            // dK[b] = scale * dscores[b]^T @ Q[b].
            //   shape: (Lk, Lq) @ (Lq, Dk) -> (Lk, Dk)
            gemm_typed<T>(true, false, static_cast<int>(Lk), static_cast<int>(Dk),
                          static_cast<int>(Lq), scale_t, dscores.data(), static_cast<int>(Lk), Qb,
                          static_cast<int>(Dk), T{0}, dKb, static_cast<int>(Dk));
        }
    };
    if (dtype_ == Dtype::F32)
        run(float{});
