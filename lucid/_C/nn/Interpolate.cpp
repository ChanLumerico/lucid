...
inline T src_coord(int out_idx, int in_dim, int out_dim, bool align_corners) {
    if (align_corners) {
        if (out_dim <= 1)
            return T{0};
        return static_cast<T>(out_idx) * static_cast<T>(in_dim - 1) / static_cast<T>(out_dim - 1);
    }
    const T x =
...
::mlx::core::array build_src_coords(int in_dim,
                                    int out_dim,
                                    bool align_corners,
                                    ::mlx::core::Dtype dt) {
    ...
    if (align_corners) {
        if (out_dim <= 1)
            return ::mlx::core::zeros({out_dim}, dt);
        auto step =
            mlx_scalar(static_cast<double>(in_dim - 1) / static_cast<double>(out_dim - 1), dt);
        return ::mlx::core::multiply(idx, step);
    }
    auto half = mlx_scalar(0.5, dt);
    auto scale = mlx_scalar(static_cast<double>(in_dim) / static_cast<double>(out_dim), dt);
...
template <typename T>
void bilinear_forward_cpu(const T* xp,
                          T* op,
                          int N,
                          int C,
                          int H_in,
...
template <typename T>
void bilinear_backward_cpu(const T* go_p,
                           T* dx_p,
                           int N,
                           int C,
                           int H_in,
...
std::vector<Storage> InterpolateBilinearBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto Hm1_i = ::mlx::core::astype(::mlx::core::array(H_in_ - 1), ::mlx::core::int32);
        auto Wm1_i = ::mlx::core::astype(::mlx::core::array(W_in_ - 1), ::mlx::core::int32);
        auto y1 = ::mlx::core::minimum(
        ...
        auto reshape_y = [&](const ::mlx::core::array& a) {
            return ::mlx::core::broadcast_to(::mlx::core::reshape(a, {1, 1, H_out_, 1}),
                                             full_idx_shape);
        };
        auto reshape_x = [&](const ::mlx::core::array& a) {
            return ::mlx::core::broadcast_to(::mlx::core::reshape(a, {1, 1, 1, W_out_}),
                                             full_idx_shape);
        };
    ...
    auto dx_cpu = allocate_size(static_cast<std::size_t>(N) * C * H_in_ * W_in_, dtype_);
    const auto& go = std::get<CpuStorage>(grad_out);
    if (dtype_ == Dtype::F32) {
        bilinear_backward_cpu<float>(reinterpret_cast<const float*>(go.ptr.get()),
                                     reinterpret_cast<float*>(dx_cpu.ptr.get()), N, C, H_in_, W_in_,
                                     H_out_, W_out_, align_corners_);
    } else if (dtype_ == Dtype::F64) {
        bilinear_backward_cpu<double>(reinterpret_cast<const double*>(go.ptr.get()),
...
template <typename T>
void trilinear_forward_cpu(const T* xp,
                           T* op,
                           int N,
                           int C,
                           int D_in,
            ...
            for (int d = 0; d < D_out; ++d) {
                ...
                for (int h = 0; h < H_out; ++h) {
                    ...
                    for (int w = 0; w < W_out; ++w) {
                        ...
                        out_base[d * H_out * W_out + h * W_out + w] = c0 * (T{1} - dz) + c1 * dz;
                    }
                }
            }
        }
...
template <typename T>
void trilinear_backward_cpu(const T* go_p,
                            T* dx_p,
                            int N,
                            int C,
                            int D_in,
...
std::vector<Storage> InterpolateTrilinearBackward::apply(Storage grad_out) {
    ...
    if (device_ == Device::GPU) {
        ...
        auto y0 = ::mlx::core::astype(y0_f, ::mlx::core::int32);
        auto x0 = ::mlx::core::astype(x0_f, ::mlx::core::int32);
        auto Dm1_i = ::mlx::core::astype(::mlx::core::array(D_in_ - 1), ::mlx::core::int32);
        auto Hm1_i = ::mlx::core::astype(::mlx::core::array(H_in_ - 1), ::mlx::core::int32);
        auto Wm1_i = ::mlx::core::astype(::mlx::core::array(W_in_ - 1), ::mlx::core::int32);
        auto z1 = ::mlx::core::minimum(
        ...
        auto reshape_z = [&](const ::mlx::core::array& a) {
            return ::mlx::core::broadcast_to(::mlx::core::reshape(a, {1, 1, D_out_, 1, 1}), full);
        };
        auto reshape_y = [&](const ::mlx::core::array& a) {
            return ::mlx::core::broadcast_to(::mlx::core::reshape(a, {1, 1, 1, H_out_, 1}), full);
        };
        auto reshape_x = [&](const ::mlx::core::array& a) {
            return ::mlx::core::broadcast_to(::mlx::core::reshape(a, {1, 1, 1, 1, W_out_}), full);
        };
    ...
    auto dx_cpu = allocate_size(static_cast<std::size_t>(N) * C * D_in_ * H_in_ * W_in_, dtype_);
    const auto& go = std::get<CpuStorage>(grad_out);
    if (dtype_ == Dtype::F32)
        trilinear_backward_cpu<float>(reinterpret_cast<const float*>(go.ptr.get()),
                                      reinterpret_cast<float*>(dx_cpu.ptr.get()), N, C, D_in_,
                                      H_in_, W_in_, D_out_, H_out_, W_out_, align_corners_);
    else if (dtype_ == Dtype::F64)
        trilinear_backward_cpu<double>(reinterpret_cast<const double*>(go.ptr.get()),
...
template <typename T>
void nearest2d_cpu(const T* xp, T* op, int N, int C, int H_in, int W_in, int H_out, int W_out) {
    for (int n = 0; n < N; ++n)
        for (int c = 0; c < C; ++c) {
            const T* base = xp + ((n * C + c) * H_in) * W_in;
            T* out_base = op + ((n * C + c) * H_out) * W_out;
            for (int h = 0; h < H_out; ++h) {
                int yh = static_cast<int>(std::floor(static_cast<double>(h) * H_in / H_out));
                yh = std::clamp(yh, 0, H_in - 1);
                for (int w = 0; w < W_out; ++w) {
                    int xw = static_cast<int>(std::floor(static_cast<double>(w) * W_in / W_out));
...
void nearest3d_cpu(const T* xp,
    ...
    for (int n = 0; n < N; ++n)
        for (int c = 0; c < C; ++c) {
            ...
            for (int d = 0; d < D_out; ++d) {
                int dz =
                    std::clamp(static_cast<int>(std::floor(static_cast<double>(d) * D_in / D_out)),
                               0, D_in - 1);
                for (int h = 0; h < H_out; ++h) {
                    int yh = std::clamp(
                        static_cast<int>(std::floor(static_cast<double>(h) * H_in / H_out)), 0,
                        H_in - 1);
                    for (int w = 0; w < W_out; ++w) {
                        int xw = std::clamp(
                            static_cast<int>(std::floor(static_cast<double>(w) * W_in / W_out)), 0,
                            W_in - 1);
                        out_base[(d * H_out + h) * W_out + w] = base[(dz * H_in + yh) * W_in + xw];
                    }
                }
            }
        }
...
TensorImplPtr interpolate_nearest_2d_op(const TensorImplPtr& input, int H_out, int W_out) {
    ...
    if (input->device() == Device::GPU) {
        ...
        ::mlx::core::array xw =
            ::mlx::core::astype(::mlx::core::arange(0, W_out, 1), ::mlx::core::float32);
        auto sH = ::mlx::core::astype(
            ::mlx::core::array(static_cast<float>(H_in) / static_cast<float>(H_out)),
            ::mlx::core::float32);
        auto sW = ::mlx::core::astype(
            ::mlx::core::array(static_cast<float>(W_in) / static_cast<float>(W_out)),
            ::mlx::core::float32);
        ...
        yh = ::mlx::core::clip(yh, std::optional<::mlx::core::array>(zero_f),
                               std::optional<::mlx::core::array>(Hm1));
        xw = ::mlx::core::clip(xw, std::optional<::mlx::core::array>(zero_f),
                               std::optional<::mlx::core::array>(Wm1));
...
TensorImplPtr interpolate_nearest_3d_op(const TensorImplPtr& input,
    ...
    if (input->device() == Device::GPU) {
        ...
        auto build_idx = [&](int in_dim, int out_dim) {
            ...
            auto scale = mlx_scalar(static_cast<double>(in_dim) / static_cast<double>(out_dim),
                                    ::mlx::core::float32);
