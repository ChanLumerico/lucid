...
TensorImplPtr DropoutBackward::forward(const TensorImplPtr& a, double p,
                                       bool training, Generator* gen) {
    if (!a) throw LucidError("dropout: null input");
    ...
    if (!training || p == 0.0) {
        ...
        auto out = std::make_shared<TensorImpl>(std::move(clone), a->shape_,
    ...
    Storage mask = bernoulli_mask_storage_shape(1.0 - p, a->shape_, a->dtype_,
                                                  a->device_, g);
    const double scale = 1.0 / (1.0 - p);

    Storage scaled_mask = mul_scalar_storage(mask, scale, numel, a->dtype_, a->device_);
    Storage y = multiply_storages(a->storage_, scaled_mask, numel,
                                  a->dtype_, a->device_);

    auto out = std::make_shared<TensorImpl>(std::move(y), a->shape_,
                                            a->dtype_, a->device_, false);
...
TensorImplPtr dropout_op(const TensorImplPtr& a, double p, bool training,
                         Generator* gen) {
...
Shape channel_mask_shape(const Shape& in) {
    if (in.size() < 2) {
        throw LucidError("dropoutnd: expected ndim >= 2, got " +
...
Shape sample_mask_shape(const Shape& in) {
    if (in.empty()) throw LucidError("drop_path: input must have ≥1 dim");
    Shape m(in.size(), 1);
    m[0] = in[0];
    return m;
}
...
template <typename T>
void expand_channel_mask_typed(const T* mask, T* out,
    ...
    for (std::size_t b = 0; b < B; ++b) {
        for (std::size_t c = 0; c < C; ++c) {
            ...
            for (std::size_t s = 0; s < spatial; ++s) dst[s] = v;
        }
    }
}

CpuStorage expand_channel_mask(const CpuStorage& mask, const Shape& full,
                                Dtype dt) {
    ...
    switch (dt) {
        case Dtype::F32:
            expand_channel_mask_typed<float>(
                reinterpret_cast<const float*>(mask.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), B, C, spatial);
            break;
        case Dtype::F64:
            expand_channel_mask_typed<double>(
                reinterpret_cast<const double*>(mask.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), B, C, spatial);
            break;
        default:
...
template <typename T>
void expand_sample_mask_typed(const T* mask, T* out,
                               std::size_t B, std::size_t per_sample) {
    for (std::size_t b = 0; b < B; ++b) {
        const T v = mask[b];
        T* dst = out + b * per_sample;
        for (std::size_t s = 0; s < per_sample; ++s) dst[s] = v;
    }
}

CpuStorage expand_sample_mask(const CpuStorage& mask, const Shape& full,
                               Dtype dt) {
    const std::size_t B = static_cast<std::size_t>(full[0]);
    std::size_t per = 1;
    ...
    switch (dt) {
        case Dtype::F32:
            expand_sample_mask_typed<float>(
                reinterpret_cast<const float*>(mask.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), B, per);
            break;
        case Dtype::F64:
            expand_sample_mask_typed<double>(
                reinterpret_cast<const double*>(mask.ptr.get()),
...
TensorImplPtr DropoutNdBackward::forward(const TensorImplPtr& a, double p,
                                          bool training, Generator* gen) {
    if (!a) throw LucidError("dropoutnd: null input");
    if (a->device_ == Device::CPU && !a->is_contiguous())
        throw NotImplementedError(
    ...
    const std::size_t mask_numel = static_cast<std::size_t>(mask_shape[0])
                                    * static_cast<std::size_t>(mask_shape[1]);
    Generator& g = gen ? *gen : default_generator();
    Storage small_mask = bernoulli_mask_storage_shape(1.0 - p, mask_shape,
                                                        a->dtype_, a->device_, g);
    const double scale = 1.0 / (1.0 - p);
    Storage scaled_small = mul_scalar_storage(small_mask, scale, mask_numel,
                                               a->dtype_, a->device_);
    Storage exp_storage;
    Storage y;
    if (a->device_ == Device::GPU) {
        ...
        CpuStorage expanded = expand_channel_mask(
            std::get<CpuStorage>(scaled_small), a->shape_, a->dtype_);
        exp_storage = Storage{std::move(expanded)};
        y = multiply_storages(a->storage_, exp_storage, numel,
                                a->dtype_, a->device_);
    }
    auto out = std::make_shared<TensorImpl>(std::move(y), a->shape_,
                                             a->dtype_, a->device_, false);
...
TensorImplPtr dropoutnd_op(const TensorImplPtr& a, double p, bool training,
                            Generator* gen) {
...
TensorImplPtr AlphaDropoutBackward::forward(const TensorImplPtr& a, double p,
                                             bool training, Generator* gen) {
    if (!a) throw LucidError("alpha_dropout: null input");
    if (a->device_ == Device::CPU && !a->is_contiguous())
        throw NotImplementedError(
            "alpha_dropout: non-contiguous input not supported");
    if (p < 0.0 || p >= 1.0)
    ...
    const double a_coef = std::pow(keep * (1.0 + p * kAlphaPrime * kAlphaPrime),
                                    -0.5);
    const double b_coef = -a_coef * p * kAlphaPrime;

    Generator& g = gen ? *gen : default_generator();
    Storage mask = bernoulli_mask_storage_shape(keep, a->shape_,
                                                  a->dtype_, a->device_, g);

    // x·mask
    Storage x_mask = multiply_storages(a->storage_, mask, numel,
                                        a->dtype_, a->device_);
    // (1 - mask)·alpha' = alpha' - alpha'·mask
    Storage alpha_mask = mul_scalar_storage(mask, kAlphaPrime, numel,
                                              a->dtype_, a->device_);
    Storage neg_alpha_mask = mul_scalar_storage(alpha_mask, -1.0, numel,
                                                  a->dtype_, a->device_);
    Storage one_m_mask_ap = add_scalar_storage(neg_alpha_mask, kAlphaPrime,
                                                 numel, a->dtype_, a->device_);
    // x·mask + (1 - mask)·alpha'
    Storage inner = add_storages(x_mask, one_m_mask_ap, numel,
                                  a->dtype_, a->device_);
    Storage scaled = mul_scalar_storage(inner, a_coef, numel,
                                          a->dtype_, a->device_);
    Storage y = add_scalar_storage(scaled, b_coef, numel,
                                    a->dtype_, a->device_);
...
std::vector<Storage> AlphaDropoutBackward::apply(Storage grad_out) {
    ...
    // dx = a_coef · mask · g
    Storage scaled_mask = mul_scalar_storage(mask_, a_coef_, numel,
                                              dtype_, device_);
    return {multiply_storages(grad_out, scaled_mask, numel, dtype_, device_)};
}

TensorImplPtr alpha_dropout_op(const TensorImplPtr& a, double p, bool training,
                                Generator* gen) {
...
template <typename T>
void dilate_seed_mask_typed(const T* seed, T* block_mask,
                             std::size_t B, std::size_t C,
...
CpuStorage build_drop_block_mask(const CpuStorage& seed, const Shape& full,
                                  std::int64_t block_size, double p, Dtype dt) {
    const std::size_t B = static_cast<std::size_t>(full[0]);
    const std::size_t C = static_cast<std::size_t>(full[1]);
    ...
    switch (dt) {
        case Dtype::F32:
            dilate_seed_mask_typed<float>(
                reinterpret_cast<const float*>(seed.ptr.get()),
                reinterpret_cast<float*>(block_mask.ptr.get()),
                B, C, H, W, block_size);
            break;
        case Dtype::F64:
            dilate_seed_mask_typed<double>(
                reinterpret_cast<const double*>(seed.ptr.get()),
                reinterpret_cast<double*>(block_mask.ptr.get()),
                B, C, H, W, block_size);
            break;
        default:
    ...
    switch (dt) {
        case Dtype::F32: {
            ...
            for (std::size_t i = 0; i < total; ++i) p32[i] = (1.f - p32[i]) * fs;
            break;
        }
        case Dtype::F64: {
            auto* p64 = reinterpret_cast<double*>(block_mask.ptr.get());
            for (std::size_t i = 0; i < total; ++i) p64[i] = (1.0 - p64[i]) * scale;
            break;
        }
        default: break;
    }
...
TensorImplPtr DropBlockBackward::forward(const TensorImplPtr& a,
                                          std::int64_t block_size, double p,
                                          double eps, Generator* gen) {
    if (!a) throw LucidError("drop_block: null input");
    if (a->device_ == Device::CPU && !a->is_contiguous())
        throw NotImplementedError(
            "drop_block: non-contiguous input not supported");
    if (a->shape_.size() != 4)
    ...
    const double valid = static_cast<double>((H - block_size + 1)
                                              * (W - block_size + 1));
...
TensorImplPtr drop_block_op(const TensorImplPtr& a, std::int64_t block_size,
...
TensorImplPtr DropPathBackward::forward(const TensorImplPtr& a, double p,
                                          bool scale_by_keep, Generator* gen) {
    if (!a) throw LucidError("drop_path: null input");
    if (a->device_ == Device::CPU && !a->is_contiguous())
        throw NotImplementedError(
            "drop_path: non-contiguous input not supported");
    if (p < 0.0 || p >= 1.0)
    ...
    Generator& g = gen ? *gen : default_generator();
    Storage small = bernoulli_mask_storage_shape(keep, sshape,
                                                   a->dtype_, a->device_, g);
    ...
    if (a->device_ == Device::GPU) {
        ...
        auto full_mask = ::mlx::core::broadcast_to(*gm.arr,
                            gpu::to_mlx_shape(a->shape_));
        const auto& gx = std::get<GpuStorage>(a->storage_);
        auto out = ::mlx::core::multiply(*gx.arr, full_mask);
        exp_storage = Storage{gpu::wrap_mlx_array(std::move(full_mask), a->dtype_)};
        y = Storage{gpu::wrap_mlx_array(std::move(out), a->dtype_)};
    } else {
        CpuStorage expanded = expand_sample_mask(
            std::get<CpuStorage>(small), a->shape_, a->dtype_);
        exp_storage = Storage{std::move(expanded)};
        y = multiply_storages(a->storage_, exp_storage, numel,
                                a->dtype_, a->device_);
    }
...
TensorImplPtr drop_path_op(const TensorImplPtr& a, double p,
                            bool scale_by_keep, Generator* gen) {
