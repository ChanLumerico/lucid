...
template <> const OpSchema ConvTranspose1dBackward::schema_v1{
    "conv_transpose1d", 1, AmpPolicy::Promote, true};
template <> const OpSchema ConvTranspose2dBackward::schema_v1{
    "conv_transpose2d", 1, AmpPolicy::Promote, true};
template <> const OpSchema ConvTranspose3dBackward::schema_v1{
    "conv_transpose3d", 1, AmpPolicy::Promote, true};
...
void im2col_dispatch(const T* x, T* cols, int C,
...
template <int N>
::mlx::core::array mlx_conv_transpose_nd(
    const ::mlx::core::array& x_nhwc,
    const ::mlx::core::array& W_nhwc,
    const int (&stride)[N], const int (&pad)[N], const int (&opad)[N]);

template <> ::mlx::core::array mlx_conv_transpose_nd<1>(
    const ::mlx::core::array& x, const ::mlx::core::array& W,
    const int (&s)[1], const int (&p)[1], const int (&op)[1]) {
    return ::mlx::core::conv_transpose1d(x, W, s[0], p[0], 1, op[0]);
}
template <> ::mlx::core::array mlx_conv_transpose_nd<2>(
    const ::mlx::core::array& x, const ::mlx::core::array& W,
    const int (&s)[2], const int (&p)[2], const int (&op)[2]) {
    return ::mlx::core::conv_transpose2d(
        x, W,
        std::pair<int, int>{s[0], s[1]},
        std::pair<int, int>{p[0], p[1]},
        std::pair<int, int>{1, 1},
        std::pair<int, int>{op[0], op[1]});
}
template <> ::mlx::core::array mlx_conv_transpose_nd<3>(
    const ::mlx::core::array& x, const ::mlx::core::array& W,
    const int (&s)[3], const int (&p)[3], const int (&op)[3]) {
    return ::mlx::core::conv_transpose3d(
        x, W,
        std::tuple<int, int, int>{s[0], s[1], s[2]},
...
template <int N>
::mlx::core::array mlx_conv_nd(
    const ::mlx::core::array& x_nhwc, const ::mlx::core::array& W_nhwc,
    const int (&stride)[N], const int (&pad)[N]);

template <> ::mlx::core::array mlx_conv_nd<1>(
    const ::mlx::core::array& x, const ::mlx::core::array& W,
    const int (&s)[1], const int (&p)[1]) {
    return ::mlx::core::conv1d(x, W, s[0], p[0]);
}
template <> ::mlx::core::array mlx_conv_nd<2>(
    const ::mlx::core::array& x, const ::mlx::core::array& W,
    const int (&s)[2], const int (&p)[2]) {
    return ::mlx::core::conv2d(x, W,
                               std::pair<int, int>{s[0], s[1]},
                               std::pair<int, int>{p[0], p[1]});
}
template <> ::mlx::core::array mlx_conv_nd<3>(
    const ::mlx::core::array& x, const ::mlx::core::array& W,
    const int (&s)[3], const int (&p)[3]) {
    return ::mlx::core::conv3d(x, W,
                               std::tuple<int, int, int>{s[0], s[1], s[2]},
                               std::tuple<int, int, int>{p[0], p[1], p[2]});
}
...
TensorImplPtr ConvTransposeNdBackward<N>::forward(
    ...
    if (b->shape_[0] != Cout)
        throw ShapeMismatch(b->shape_, W->shape_,
                            "conv_transpose: bias C_out mismatch");
    ...
    for (int i = 0; i < N; ++i) {
        O_total *= O[i]; S_total *= S[i]; K_total *= K[i];
    }

    OpScope scope{ConvTransposeNdBackward<N>::schema_v1.name, x->device_,
                  x->dtype_, out_shape};
    ...
    if (x->device_ == Device::GPU) {
        ...
        auto cols_cpu = allocate_size(static_cast<std::size_t>(K_flat) * S_total,
                                      x->dtype_);
            ...
            switch (x->dtype_) {
                ...
                case Dtype::F64: {
                    ...
                    auto* yp = reinterpret_cast<double*>(out_cpu.ptr.get())
                               + static_cast<std::size_t>(bi) * Cout * O_total;
                    backend::cpu::dgemm(
                        true, false, K_flat, S_total, Cin, 1.0,
                        Wp, K_flat, xp, S_total, 0.0, cp, S_total);
                ...
                default:
                    throw NotImplementedError(
                        "conv_transpose: dtype not supported (F32/F64)");
            }
        }
...
std::vector<Storage> ConvTransposeNdBackward<N>::apply(Storage grad_out) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        S_total *= S[i]; K_total *= K[i]; O_total *= O[i];
    }
    ...
    if (this->device_ == Device::GPU) {
        ...
        auto dx_nhwc = mlx_conv_nd<N>(grad_nhwc, W_dx_nhwc,
        ...
        auto dW_perm = ::mlx::core::conv_general(
            g_perm, x_perm, conv_stride, conv_pad, conv_pad,
            conv_kdil, conv_idil);
        // Crop to (Co, *K, Ci).
    ...
    for (int bi = 0; bi < B; ++bi) {
        switch (this->dtype_) {
            case Dtype::F32: {
                ...
                im2col_dispatch<N, float>(gp, cp, Cout, O, K, S,
                                          this->stride_, this->pad_);

                // dx_2d (Cin, prod(S)) = W_2d (Cin, K_flat) @ cols (K_flat, prod(S))
                backend::cpu::sgemm(
                    false, false, Cin, S_total, K_flat, 1.0f,
                    reinterpret_cast<const float*>(W_cpu.ptr.get()), K_flat,
                    cp, S_total, 0.0f, dxp, S_total);

                // dW_2d (Cin, K_flat) += x_2d (Cin, S_total) @ cols^T (S_total, K_flat)
                backend::cpu::sgemm(
                    false, true, Cin, K_flat, S_total, 1.0f,
            ...
            case Dtype::F64: {
                ...
                im2col_dispatch<N, double>(gp, cp, Cout, O, K, S,
                                           this->stride_, this->pad_);
                backend::cpu::dgemm(
                    false, false, Cin, S_total, K_flat, 1.0,
                    reinterpret_cast<const double*>(W_cpu.ptr.get()), K_flat,
                    cp, S_total, 0.0, dxp, S_total);
                backend::cpu::dgemm(
                    false, true, Cin, K_flat, S_total, 1.0,
...
TensorImplPtr conv_transpose1d_op(
    const TensorImplPtr& x, const TensorImplPtr& W, const TensorImplPtr& b,
    int sl, int pl, int opl) {
    int s[1]{sl}; int p[1]{pl}; int op[1]{opl};
    return ConvTranspose1dBackward::forward(x, W, b, s, p, op);
}
TensorImplPtr conv_transpose2d_op(
    const TensorImplPtr& x, const TensorImplPtr& W, const TensorImplPtr& b,
    int sh, int sw, int ph, int pw, int oph, int opw) {
    int s[2]{sh, sw}; int p[2]{ph, pw}; int op[2]{oph, opw};
    return ConvTranspose2dBackward::forward(x, W, b, s, p, op);
}
TensorImplPtr conv_transpose3d_op(
    const TensorImplPtr& x, const TensorImplPtr& W, const TensorImplPtr& b,
    int sd, int sh, int sw, int pd, int ph, int pw,
    int opd, int oph, int opw) {
    int s[3]{sd, sh, sw}; int p[3]{pd, ph, pw}; int op[3]{opd, oph, opw};
