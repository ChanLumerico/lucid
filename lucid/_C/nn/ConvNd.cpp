...
void im2col_dispatch<1, float>(const float* x,
                               ...
                               const int* pad,
                               const int* dil) {
    backend::cpu::im2col_1d_f32(x, cols, C, S[0], K[0], O[0], stride[0], pad[0], dil[0]);
}
template <>
void im2col_dispatch<1, double>(const double* x,
                                ...
                                const int* pad,
                                const int* dil) {
    backend::cpu::im2col_1d_f64(x, cols, C, S[0], K[0], O[0], stride[0], pad[0], dil[0]);
}
template <>
...
void col2im_dispatch<1, float>(const float* cols,
                               ...
                               const int* pad,
                               const int* dil) {
    backend::cpu::col2im_1d_f32(cols, dx, C, S[0], K[0], O[0], stride[0], pad[0], dil[0]);
}
template <>
void col2im_dispatch<1, double>(const double* cols,
                                ...
                                const int* pad,
                                const int* dil) {
    backend::cpu::col2im_1d_f64(cols, dx, C, S[0], K[0], O[0], stride[0], pad[0], dil[0]);
}
template <>
...
template <int N>
std::vector<int> w_nchw_to_nhwc_perm() {
    return nchw_to_nhwc_perm<N>();
}
                    ...
                    case Dtype::F32: {
                        auto* xp = reinterpret_cast<const float*>(x_cpu.ptr.get()) +
                                   (static_cast<std::size_t>(bi) * Cin +
                                    static_cast<std::size_t>(g) * Cin_g) *
                                       S_total;
                        ...
                        auto* yp = reinterpret_cast<float*>(out_cpu.ptr.get()) +
                                   (static_cast<std::size_t>(bi) * Cout +
                                    static_cast<std::size_t>(g) * Cout_g) *
                                       O_total;
                        im2col_dispatch<N, float>(xp, cp, Cin_g, S, K, O, stride, pad, dilation);
                        backend::cpu::sgemm(false, false, Cout_g, M_out, K_flat, 1.0f, wp, K_flat,
                                            cp, M_out, 0.0f, yp, M_out);
                        break;
                    }
                    case Dtype::F64: {
                        ...
                        auto* yp = reinterpret_cast<double*>(out_cpu.ptr.get()) +
                                   (static_cast<std::size_t>(bi) * Cout +
                                    static_cast<std::size_t>(g) * Cout_g) *
                                       O_total;
                        im2col_dispatch<N, double>(xp, cp, Cin_g, S, K, O, stride, pad, dilation);
                        backend::cpu::dgemm(false, false, Cout_g, M_out, K_flat, 1.0, wp, K_flat,
                                            cp, M_out, 0.0, yp, M_out);
                        break;
                    }
                    default:
...
std::vector<Storage> ConvNdBackward<N>::apply(Storage grad_out) {
    ...
    if (this->device_ == Device::GPU) {
        ...
        const std::vector<int>& conv_pad = pv;
        const std::vector<int>& conv_kdil = sv;
        const std::vector<int>& conv_idil = dv;
        auto dW_perm =
            ::mlx::core::conv_general(x_perm, g_perm, conv_stride, conv_pad, conv_pad, conv_kdil,
                                      conv_idil, /*groups=*/G, /*flip=*/false);
        ...
        for (int i = 0; i < N; ++i)
            dW_perm_back.push_back(1 + i);
        auto dW = ::mlx::core::contiguous(::mlx::core::transpose(dW_perm, dW_perm_back));
        ...
        for (int g = 0; g < G; ++g) {
            switch (this->dtype_) {
                case Dtype::F32: {
                    ...
                    im2col_dispatch<N, float>(xp, cp, Cin_g, S, K, O, this->stride_, this->pad_,
                                              this->dilation_);
                    backend::cpu::sgemm(false, true, Cout_g, K_flat, M_out, 1.0f, gp, M_out, cp,
                                        M_out, 1.0f, dwp, K_flat);
                    backend::cpu::sgemm(true, false, K_flat, M_out, Cout_g, 1.0f, wp, K_flat, gp,
                                        M_out, 0.0f, cgp, M_out);
                    col2im_dispatch<N, float>(cgp, dxp, Cin_g, S, K, O, this->stride_, this->pad_,
                                              this->dilation_);
                    {
                ...
                case Dtype::F64: {
                    ...
                    im2col_dispatch<N, double>(xp, cp, Cin_g, S, K, O, this->stride_, this->pad_,
                                               this->dilation_);
                    backend::cpu::dgemm(false, true, Cout_g, K_flat, M_out, 1.0, gp, M_out, cp,
                                        M_out, 1.0, dwp, K_flat);
                    backend::cpu::dgemm(true, false, K_flat, M_out, Cout_g, 1.0, wp, K_flat, gp,
                                        M_out, 0.0, cgp, M_out);
                    col2im_dispatch<N, double>(cgp, dxp, Cin_g, S, K, O, this->stride_, this->pad_,
                                               this->dilation_);
                    {
...
TensorImplPtr UnfoldBackward::forward(const TensorImplPtr& x,
    ...
    if (x->device() == Device::GPU) {
        ...
        for (int d = 0; d < N; ++d)
            composite_shape.push_back(K[d]);
        for (int d = 0; d < N; ++d)
            composite_shape.push_back(O[d]);
        ...
        auto x_flat = ::mlx::core::reshape(*gx.arr, ::mlx::core::Shape{B * C * S_total});
        auto sampled = ::mlx::core::take(x_flat, flat);
        auto valid_b =
            ::mlx::core::astype(::mlx::core::broadcast_to(valid, composite_shape), mlx_dt);
    ...
    for (int bi = 0; bi < B; ++bi) {
        switch (x->dtype()) {
            case Dtype::F32: {
                ...
                if (N == 1)
                    backend::cpu::im2col_1d_f32(xp, yp, C, S[0], K[0], O[0], stride[0], pad[0],
                                                dilation[0]);
                else if (N == 2)
                    backend::cpu::im2col_f32(xp, yp, C, S[0], S[1], K[0], K[1], O[0], O[1],
                                             stride[0], stride[1], pad[0], pad[1], dilation[0],
                                             dilation[1]);
                else
                    backend::cpu::im2col_3d_f32(xp, yp, C, S[0], S[1], S[2], K[0], K[1], K[2], O[0],
                                                O[1], O[2], stride[0], stride[1], stride[2], pad[0],
                                                pad[1], pad[2], dilation[0], dilation[1],
                                                dilation[2]);
                break;
            }
            case Dtype::F64: {
                ...
                auto* yp = reinterpret_cast<double*>(out_cpu.ptr.get()) +
                           static_cast<std::size_t>(bi) * C * K_total * O_total;
                if (N == 1)
                    backend::cpu::im2col_1d_f64(xp, yp, C, S[0], K[0], O[0], stride[0], pad[0],
                                                dilation[0]);
                else if (N == 2)
                    backend::cpu::im2col_f64(xp, yp, C, S[0], S[1], K[0], K[1], O[0], O[1],
                                             stride[0], stride[1], pad[0], pad[1], dilation[0],
                                             dilation[1]);
                else
                    backend::cpu::im2col_3d_f64(xp, yp, C, S[0], S[1], S[2], K[0], K[1], K[2], O[0],
                                                O[1], O[2], stride[0], stride[1], stride[2], pad[0],
                                                pad[1], pad[2], dilation[0], dilation[1],
                                                dilation[2]);
                break;
...
std::vector<Storage> UnfoldBackward::apply(Storage grad_out) {
    ...
    if (this->device_ == Device::GPU) {
        ...
        for (int d = 0; d < N; ++d)
            idxs.push_back(::mlx::core::broadcast_to(in_d_clipped[d], composite_shape));
        ...
        switch (this->dtype_) {
            case Dtype::F32: {
                ...
                auto* dxp = reinterpret_cast<float*>(dx_cpu.ptr.get()) +
                            static_cast<std::size_t>(bi) * C * S_total;
                if (N == 1)
                    backend::cpu::col2im_1d_f32(gp, dxp, C, S[0], K[0], O[0], stride_[0], pad_[0],
                                                dilation_[0]);
                else if (N == 2)
                    backend::cpu::col2im_f32(gp, dxp, C, S[0], S[1], K[0], K[1], O[0], O[1],
                                             stride_[0], stride_[1], pad_[0], pad_[1], dilation_[0],
                                             dilation_[1]);
                else
                    backend::cpu::col2im_3d_f32(gp, dxp, C, S[0], S[1], S[2], K[0], K[1], K[2],
                                                O[0], O[1], O[2], stride_[0], stride_[1],
                                                stride_[2], pad_[0], pad_[1], pad_[2], dilation_[0],
            ...
            case Dtype::F64: {
                ...
                auto* dxp = reinterpret_cast<double*>(dx_cpu.ptr.get()) +
                            static_cast<std::size_t>(bi) * C * S_total;
                if (N == 1)
                    backend::cpu::col2im_1d_f64(gp, dxp, C, S[0], K[0], O[0], stride_[0], pad_[0],
                                                dilation_[0]);
                else if (N == 2)
                    backend::cpu::col2im_f64(gp, dxp, C, S[0], S[1], K[0], K[1], O[0], O[1],
                                             stride_[0], stride_[1], pad_[0], pad_[1], dilation_[0],
                                             dilation_[1]);
                else
                    backend::cpu::col2im_3d_f64(gp, dxp, C, S[0], S[1], S[2], K[0], K[1], K[2],
                                                O[0], O[1], O[2], stride_[0], stride_[1],
                                                stride_[2], pad_[0], pad_[1], pad_[2], dilation_[0],
                                                dilation_[1], dilation_[2]);
                break;
