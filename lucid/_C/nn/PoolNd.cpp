...
template <> const OpSchema MaxPool1dBackward::schema_v1{
    "max_pool1d", 1, AmpPolicy::KeepInput, true};
template <> const OpSchema MaxPool2dBackward::schema_v1{
    "max_pool2d", 1, AmpPolicy::KeepInput, true};
template <> const OpSchema MaxPool3dBackward::schema_v1{
    "max_pool3d", 1, AmpPolicy::KeepInput, true};
template <> const OpSchema AvgPool1dBackward::schema_v1{
    "avg_pool1d", 1, AmpPolicy::KeepInput, true};
template <> const OpSchema AvgPool2dBackward::schema_v1{
    "avg_pool2d", 1, AmpPolicy::KeepInput, true};
template <> const OpSchema AvgPool3dBackward::schema_v1{
    "avg_pool3d", 1, AmpPolicy::KeepInput, true};
...
void validate_input(const TensorImplPtr& x, std::string_view op_name) {
    if (!x) throw LucidError(std::string(op_name) + ": null input");
    if (x->device_ == Device::CPU && !x->is_contiguous())
        throw NotImplementedError(std::string(op_name) +
                                  ": non-contiguous input not supported");
    if (static_cast<int>(x->shape_.size()) != N + 2)
        throw ShapeMismatch(x->shape_, Shape{},
                            std::string(op_name) + ": x rank mismatch");
}

// CPU forward dispatch by N — picks the right backend kernel.
template <int N, typename T>
void max_pool_forward_dispatch(const T* x, T* y, std::int32_t* a,
                               int B, int C, const int* S,
                               const int* K, const int* O,
                               const int* stride, const int* pad);
template <> void max_pool_forward_dispatch<1, float>(
    const float* x, float* y, std::int32_t* a, int B, int C,
    const int* S, const int* K, const int* O,
    const int* stride, const int* pad) {
    backend::cpu::max_pool1d_forward_f32(x, y, a, B, C, S[0], K[0], O[0],
                                         stride[0], pad[0]);
}
template <> void max_pool_forward_dispatch<1, double>(
    const double* x, double* y, std::int32_t* a, int B, int C,
    const int* S, const int* K, const int* O,
    const int* stride, const int* pad) {
    backend::cpu::max_pool1d_forward_f64(x, y, a, B, C, S[0], K[0], O[0],
                                         stride[0], pad[0]);
}
template <> void max_pool_forward_dispatch<2, float>(
    const float* x, float* y, std::int32_t* a, int B, int C,
    const int* S, const int* K, const int* O,
    const int* stride, const int* pad) {
    backend::cpu::max_pool2d_forward_f32(x, y, a, B, C, S[0], S[1],
                                         K[0], K[1], O[0], O[1],
                                         stride[0], stride[1], pad[0], pad[1]);
}
template <> void max_pool_forward_dispatch<2, double>(
    const double* x, double* y, std::int32_t* a, int B, int C,
    const int* S, const int* K, const int* O,
    const int* stride, const int* pad) {
    backend::cpu::max_pool2d_forward_f64(x, y, a, B, C, S[0], S[1],
                                         K[0], K[1], O[0], O[1],
                                         stride[0], stride[1], pad[0], pad[1]);
}
template <> void max_pool_forward_dispatch<3, float>(
    const float* x, float* y, std::int32_t* a, int B, int C,
    const int* S, const int* K, const int* O,
    const int* stride, const int* pad) {
    backend::cpu::max_pool3d_forward_f32(x, y, a, B, C, S[0], S[1], S[2],
                                         K[0], K[1], K[2], O[0], O[1], O[2],
                                         stride[0], stride[1], stride[2],
                                         pad[0], pad[1], pad[2]);
}
template <> void max_pool_forward_dispatch<3, double>(
    const double* x, double* y, std::int32_t* a, int B, int C,
    const int* S, const int* K, const int* O,
    const int* stride, const int* pad) {
    backend::cpu::max_pool3d_forward_f64(x, y, a, B, C, S[0], S[1], S[2],
                                         K[0], K[1], K[2], O[0], O[1], O[2],
                                         stride[0], stride[1], stride[2],
                                         pad[0], pad[1], pad[2]);
}
...
template <int N, typename T>
void max_pool_backward_dispatch(const T* g, const std::int32_t* a, T* dx,
                                int B, int C, const int* S, const int* O);
template <> void max_pool_backward_dispatch<1, float>(
    const float* g, const std::int32_t* a, float* dx,
    int B, int C, const int* S, const int* O) {
    backend::cpu::max_pool1d_backward_f32(g, a, dx, B, C, S[0], O[0]);
}
template <> void max_pool_backward_dispatch<1, double>(
    const double* g, const std::int32_t* a, double* dx,
    int B, int C, const int* S, const int* O) {
    backend::cpu::max_pool1d_backward_f64(g, a, dx, B, C, S[0], O[0]);
}
template <> void max_pool_backward_dispatch<2, float>(
    const float* g, const std::int32_t* a, float* dx,
    int B, int C, const int* S, const int* O) {
    backend::cpu::max_pool2d_backward_f32(g, a, dx, B, C, S[0], S[1], O[0], O[1]);
}
template <> void max_pool_backward_dispatch<2, double>(
    const double* g, const std::int32_t* a, double* dx,
    int B, int C, const int* S, const int* O) {
    backend::cpu::max_pool2d_backward_f64(g, a, dx, B, C, S[0], S[1], O[0], O[1]);
}
template <> void max_pool_backward_dispatch<3, float>(
    const float* g, const std::int32_t* a, float* dx,
    int B, int C, const int* S, const int* O) {
    backend::cpu::max_pool3d_backward_f32(g, a, dx, B, C, S[0], S[1], S[2],
                                          O[0], O[1], O[2]);
}
template <> void max_pool_backward_dispatch<3, double>(
    const double* g, const std::int32_t* a, double* dx,
    int B, int C, const int* S, const int* O) {
    backend::cpu::max_pool3d_backward_f64(g, a, dx, B, C, S[0], S[1], S[2],
                                          O[0], O[1], O[2]);
}

template <int N, typename T>
void avg_pool_forward_dispatch(const T* x, T* y,
                               int B, int C, const int* S,
                               const int* K, const int* O,
                               const int* stride, const int* pad);
template <> void avg_pool_forward_dispatch<1, float>(
    const float* x, float* y, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool1d_forward_f32(x, y, B, C, S[0], K[0], O[0],
                                         stride[0], pad[0]);
}
template <> void avg_pool_forward_dispatch<1, double>(
    const double* x, double* y, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool1d_forward_f64(x, y, B, C, S[0], K[0], O[0],
                                         stride[0], pad[0]);
}
template <> void avg_pool_forward_dispatch<2, float>(
    const float* x, float* y, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool2d_forward_f32(x, y, B, C, S[0], S[1],
                                         K[0], K[1], O[0], O[1],
                                         stride[0], stride[1], pad[0], pad[1]);
}
template <> void avg_pool_forward_dispatch<2, double>(
    const double* x, double* y, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool2d_forward_f64(x, y, B, C, S[0], S[1],
                                         K[0], K[1], O[0], O[1],
                                         stride[0], stride[1], pad[0], pad[1]);
}
template <> void avg_pool_forward_dispatch<3, float>(
    const float* x, float* y, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool3d_forward_f32(x, y, B, C, S[0], S[1], S[2],
                                         K[0], K[1], K[2], O[0], O[1], O[2],
                                         stride[0], stride[1], stride[2],
                                         pad[0], pad[1], pad[2]);
}
template <> void avg_pool_forward_dispatch<3, double>(
    const double* x, double* y, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool3d_forward_f64(x, y, B, C, S[0], S[1], S[2],
                                         K[0], K[1], K[2], O[0], O[1], O[2],
                                         stride[0], stride[1], stride[2],
                                         pad[0], pad[1], pad[2]);
}
...
template <int N, typename T>
void avg_pool_backward_dispatch(const T* g, T* dx,
                                int B, int C, const int* S,
                                const int* K, const int* O,
                                const int* stride, const int* pad);
template <> void avg_pool_backward_dispatch<1, float>(
    const float* g, float* dx, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool1d_backward_f32(g, dx, B, C, S[0], K[0], O[0],
                                          stride[0], pad[0]);
}
template <> void avg_pool_backward_dispatch<1, double>(
    const double* g, double* dx, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool1d_backward_f64(g, dx, B, C, S[0], K[0], O[0],
                                          stride[0], pad[0]);
}
template <> void avg_pool_backward_dispatch<2, float>(
    const float* g, float* dx, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool2d_backward_f32(g, dx, B, C, S[0], S[1],
                                          K[0], K[1], O[0], O[1],
                                          stride[0], stride[1], pad[0], pad[1]);
}
template <> void avg_pool_backward_dispatch<2, double>(
    const double* g, double* dx, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool2d_backward_f64(g, dx, B, C, S[0], S[1],
                                          K[0], K[1], O[0], O[1],
                                          stride[0], stride[1], pad[0], pad[1]);
}
template <> void avg_pool_backward_dispatch<3, float>(
    const float* g, float* dx, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool3d_backward_f32(g, dx, B, C, S[0], S[1], S[2],
                                          K[0], K[1], K[2], O[0], O[1], O[2],
                                          stride[0], stride[1], stride[2],
                                          pad[0], pad[1], pad[2]);
}
template <> void avg_pool_backward_dispatch<3, double>(
    const double* g, double* dx, int B, int C, const int* S,
    const int* K, const int* O, const int* stride, const int* pad) {
    backend::cpu::avg_pool3d_backward_f64(g, dx, B, C, S[0], S[1], S[2],
                                          K[0], K[1], K[2], O[0], O[1], O[2],
                                          stride[0], stride[1], stride[2],
                                          pad[0], pad[1], pad[2]);
}
...
TensorImplPtr MaxPoolNdBackward<N>::forward(const TensorImplPtr& x,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (O[i] <= 0)
            throw ShapeMismatch(x->shape_, Shape{},
                                "max_pool: output non-positive");
        O_total *= O[i];
        S_total *= S[i];
    }
    int K_total = 1;
    for (int i = 0; i < N; ++i) K_total *= K[i];
    ...
    if (x->device_ == Device::GPU) {
        ...
        if (!gx.arr) throw LucidError("max_pool: null GPU input");
        ...
        pad_widths.emplace_back(0, 0);
        for (int i = 0; i < N; ++i) pad_widths.emplace_back(pad[i], pad[i]);
        auto x_pad = ::mlx::core::pad(*gx.arr, pad_widths, neg_inf);

        int Sp[N];
        for (int i = 0; i < N; ++i) Sp[i] = S[i] + 2 * pad[i];
        ...
        saved_argmax = Storage{
            gpu::wrap_mlx_array(std::move(argmax), Dtype::I32)};
    } else {
        ...
        switch (x->dtype_) {
            case Dtype::F32:
                max_pool_forward_dispatch<N, float>(
                    ...
                    reinterpret_cast<std::int32_t*>(am_cpu.ptr.get()),
                    B, C, S, K, O, stride, pad);
                break;
            case Dtype::F64:
    ...
    for (int i = 0; i < N; ++i) {
        bwd->K_[i] = K[i]; bwd->stride_[i] = stride[i]; bwd->pad_[i] = pad[i];
    }
...
std::vector<Storage> MaxPoolNdBackward<N>::apply(Storage grad_out) {
    ...
    if (this->device_ == Device::GPU) {
        ...
        int Sp[N];
        for (int i = 0; i < N; ++i) Sp[i] = S[i] + 2 * this->pad_[i];
        ...
        auto compute_ih = [&](int i) -> ::mlx::core::array {
            ::mlx::core::array div_arr(
                static_cast<std::int32_t>(K_suffix[i + 1]), idt);
            ::mlx::core::array mod_arr(
                static_cast<std::int32_t>(this->K_[i]), idt);
            auto ki = ::mlx::core::remainder(
                ::mlx::core::floor_divide(*gA.arr, div_arr), mod_arr);
            ::mlx::core::Shape range_shape(N + 2, 1);
            range_shape[2 + i] = static_cast<SE>(O[i]);
            auto o_range = ::mlx::core::reshape(
                ::mlx::core::arange(0, O[i], 1, idt), range_shape);
            ::mlx::core::array stride_arr(
                static_cast<std::int32_t>(this->stride_[i]), idt);
            return ::mlx::core::add(
                ::mlx::core::multiply(o_range, stride_arr), ki);
        };

        ::mlx::core::array flat_idx = compute_ih(0);
        for (int i = 1; i < N; ++i) {
            ::mlx::core::array Sp_arr(
                static_cast<std::int32_t>(Sp[i]), idt);
            flat_idx = ::mlx::core::add(
                ::mlx::core::multiply(flat_idx, Sp_arr), compute_ih(i));
        }
        ...
        for (int i = 0; i < N; ++i) full_O.push_back(static_cast<SE>(O[i]));
        ...
        ::mlx::core::array zero_pad =
            ::mlx::core::zeros(pad_2d, gpu::to_mlx_dtype(this->dtype_));
        auto dx_pad_2d = ::mlx::core::scatter_add_axis(
            zero_pad, idx_2d, g_2d, /*axis=*/1);
        ...
        for (int i = 0; i < N; ++i) full_Sp.push_back(static_cast<SE>(Sp[i]));
...
TensorImplPtr AvgPoolNdBackward<N>::forward(const TensorImplPtr& x,
    ...
    if (x->device_ == Device::GPU) {
        const auto& gx = std::get<GpuStorage>(x->storage_);
        if (!gx.arr) throw LucidError("avg_pool: null GPU input");
        ...
        switch (x->dtype_) {
            case Dtype::F32:
                avg_pool_forward_dispatch<N, float>(
                    reinterpret_cast<const float*>(x_cpu.ptr.get()),
                    reinterpret_cast<float*>(y_cpu.ptr.get()),
                    B, C, S, K, O, stride, pad);
                break;
...
std::vector<Storage> AvgPoolNdBackward<N>::apply(Storage grad_out) {
    ...
    if (this->device_ == Device::GPU) {
        const auto& gG = std::get<GpuStorage>(grad_out);
        if (!gG.arr) throw LucidError("avg_pool backward: null GPU array");
        using SE = ::mlx::core::ShapeElem;
        const auto idt = ::mlx::core::int32;
        const int K = K_total;
        int Sp[N];
        ...
        ::mlx::core::array inv_K(1.0 / static_cast<double>(K),
                                 gpu::to_mlx_dtype(this->dtype_));
        ::mlx::core::Shape g_with_k;
        g_with_k.reserve(N + 3);
        g_with_k.push_back(static_cast<SE>(B));
        g_with_k.push_back(static_cast<SE>(C));
        for (int i = 0; i < N; ++i) g_with_k.push_back(static_cast<SE>(O[i]));
        ...
        ::mlx::core::array k_range = ::mlx::core::reshape(
            ::mlx::core::arange(0, K, 1, idt), kr_shape);
        ...
        auto compute_ih = [&](int i) -> ::mlx::core::array {
            ...
            ::mlx::core::array mod_arr(
                static_cast<std::int32_t>(this->K_[i]), idt);
            auto ki = ::mlx::core::remainder(
                ::mlx::core::floor_divide(k_range, div_arr), mod_arr);
    ...
    switch (this->dtype_) {
        case Dtype::F32:
            avg_pool_backward_dispatch<N, float>(
                reinterpret_cast<const float*>(g_cpu.ptr.get()),
                reinterpret_cast<float*>(dx_cpu.ptr.get()),
                B, C, S, this->K_, O, this->stride_, this->pad_);
            break;
        case Dtype::F64:
            avg_pool_backward_dispatch<N, double>(
                reinterpret_cast<const double*>(g_cpu.ptr.get()),
                reinterpret_cast<double*>(dx_cpu.ptr.get()),
                B, C, S, this->K_, O, this->stride_, this->pad_);
            break;
...
TensorImplPtr max_pool1d_op(const TensorImplPtr& x, int KL, int sl, int pl) {
    int K[1]{KL}; int s[1]{sl}; int p[1]{pl};
    return MaxPool1dBackward::forward(x, K, s, p);
}
TensorImplPtr max_pool2d_op(const TensorImplPtr& x, int KH, int KW,
                            int sh, int sw, int ph, int pw) {
    int K[2]{KH, KW}; int s[2]{sh, sw}; int p[2]{ph, pw};
    return MaxPool2dBackward::forward(x, K, s, p);
}
TensorImplPtr max_pool3d_op(const TensorImplPtr& x, int KD, int KH, int KW,
                            int sd, int sh, int sw, int pd, int ph, int pw) {
    int K[3]{KD, KH, KW}; int s[3]{sd, sh, sw}; int p[3]{pd, ph, pw};
...
TensorImplPtr avg_pool2d_op(const TensorImplPtr& x, int KH, int KW,
                            int sh, int sw, int ph, int pw) {
    int K[2]{KH, KW}; int s[2]{sh, sw}; int p[2]{ph, pw};
    return AvgPool2dBackward::forward(x, K, s, p);
}
TensorImplPtr avg_pool3d_op(const TensorImplPtr& x, int KD, int KH, int KW,
                            int sd, int sh, int sw, int pd, int ph, int pw) {
    int K[3]{KD, KH, KW}; int s[3]{sd, sh, sw}; int p[3]{pd, ph, pw};
