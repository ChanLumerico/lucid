...
const OpSchema RMSNormBackward::schema_v1{
    "rms_norm", 1, AmpPolicy::ForceFP32, true};
...
TensorImplPtr RMSNormBackward::forward(const TensorImplPtr& x,
                                       const TensorImplPtr& gamma, double eps) {
    if (!x || !gamma) throw LucidError("rms_norm: null input");
    ...
    for (std::size_t i = 0; i < lead; ++i) outer *= static_cast<std::size_t>(x->shape_[i]);
    for (std::size_t i = 0; i < Dn; ++i) N *= static_cast<std::size_t>(gamma->shape_[i]);
...
std::vector<Storage> RMSNormBackward::apply(Storage grad_out) {
    if (device_ == Device::GPU) {
        ...
        auto dx_2d = ::mlx::core::multiply(
            *gR.arr,
            ::mlx::core::subtract(gx_scaled, ::mlx::core::multiply(xnorm, m)));
...
TensorImplPtr rms_norm_op(const TensorImplPtr& x, const TensorImplPtr& gamma,
                          double eps) {
