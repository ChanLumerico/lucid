...
public:
    ...
    static TensorImplPtr forward(const TensorImplPtr& x, double ord,
                                 int axis, double eps);
    std::vector<Storage> apply(Storage grad_out) override;
};

LUCID_API TensorImplPtr lp_normalize_op(const TensorImplPtr& x, double ord,
                                         int axis, double eps);

// ----- Global Response Norm -----
class LUCID_API GlobalResponseNormBackward
    : public FuncOp<GlobalResponseNormBackward, 3> {
public:
    static const OpSchema schema_v1;
    double eps_ = 1e-6;
