...
constexpr const char* kGpuStubMessage =
    "GPU storage helpers are not yet wired (Phase 3 enables MLX integration).";

template <typename T>
void fill_typed(std::byte* dst, std::size_t numel, T value) {
    auto* tdst = reinterpret_cast<T*>(dst);
    for (std::size_t i = 0; i < numel; ++i) tdst[i] = value;
}
...
void fill_storage(CpuStorage& s, const Shape& shape, double value) {
    ...
    switch (s.dtype) {
        ...
        case Dtype::I8:
            fill_typed<std::int8_t>(s.ptr.get(), n,
                                    static_cast<std::int8_t>(value));
            break;
        case Dtype::I16:
            fill_typed<std::int16_t>(s.ptr.get(), n,
                                     static_cast<std::int16_t>(value));
            break;
        case Dtype::I32:
            fill_typed<std::int32_t>(s.ptr.get(), n,
                                     static_cast<std::int32_t>(value));
            break;
        case Dtype::I64:
            fill_typed<std::int64_t>(s.ptr.get(), n,
...
void add_typed(std::byte* dst, const std::byte* src, std::size_t numel) {
    ...
    for (std::size_t i = 0; i < numel; ++i) td[i] = td[i] + ts[i];
}

void cpu_add_inplace(CpuStorage& dst, const CpuStorage& src) {
    if (dst.dtype != src.dtype) {
        throw DtypeMismatch(std::string(dtype_name(dst.dtype)),
    ...
    switch (dst.dtype) {
        ...
        default:
            throw NotImplementedError(
                "accumulate_into: dtype not yet supported in Phase 2");
    }
}
...
std::vector<std::size_t> broadcast_reduce_axes(const Shape& grad_shape,
    ...
    for (std::size_t i = 0; i < lead; ++i) axes.push_back(i);
...
template <typename T>
void reduce_axes_typed(const std::byte* src_bytes, std::byte* dst_bytes,
    ...
    for (auto a : reduce_axes_set) reduce_mask[a] = true;

    Stride grad_idx_stride(gn);
    if (gn > 0) {
        grad_idx_stride[gn - 1] = 1;
        for (std::ptrdiff_t i = static_cast<std::ptrdiff_t>(gn) - 2; i >= 0;
             --i) {
    ...
    if (tn > 0) {
        ...
        for (std::ptrdiff_t i = static_cast<std::ptrdiff_t>(tn) - 2; i >= 0;
             --i) {
    ...
    for (std::size_t flat = 0; flat < grad_numel; ++flat) {
        ...
        for (std::size_t d = 0; d < gn; ++d) {
            const std::size_t coord =
                rem / static_cast<std::size_t>(grad_idx_stride[d]);
            rem -= coord * static_cast<std::size_t>(grad_idx_stride[d]);
            if (d < lead) continue;             // squashed by leading-1 padding
            if (reduce_mask[d]) continue;       // collapsed axis
...
Storage make_zero_storage(const Shape& shape, Dtype dtype, Device device) {
    if (device == Device::GPU) {
        auto out = ::mlx::core::zeros(gpu::to_mlx_shape(shape),
                                      gpu::to_mlx_dtype(dtype));
...
Storage make_ones_storage(const Shape& shape, Dtype dtype, Device device) {
    if (device == Device::GPU) {
        auto out = ::mlx::core::ones(gpu::to_mlx_shape(shape),
                                     gpu::to_mlx_dtype(dtype));
        return Storage{gpu::wrap_mlx_array(std::move(out), dtype)};
    }
    auto cpu = allocate_cpu(shape, dtype);
...
Storage reduce_grad_to_shape(const Storage& grad, const Shape& grad_shape,
    ...
    if (device == Device::GPU) {
        ...
        for (auto a : reduce_axes) axes_i.push_back(static_cast<int>(a));
        ...
        auto reshaped =
            ::mlx::core::reshape(reduced, gpu::to_mlx_shape(target_shape));
    ...
    switch (dtype) {
        case Dtype::F32:
            reduce_axes_typed<float>(src_cpu.ptr.get(), out.ptr.get(),
                                     grad_shape, target_shape, reduce_axes);
            break;
        case Dtype::F64:
            reduce_axes_typed<double>(src_cpu.ptr.get(), out.ptr.get(),
                                      grad_shape, target_shape, reduce_axes);
            break;
        default:
            throw NotImplementedError(
                "reduce_grad_to_shape: dtype not yet supported in Phase 2");
    }
    return Storage{std::move(out)};
}
...
void accumulate_into(Storage& dst, const Storage& src) {
    std::visit(
        overloaded{
            ...
            [&](auto&, auto&) {
                throw DeviceMismatch("matching device", "mixed CPU/GPU",
                                     "accumulate_into");
            },
        },
        dst, src);
...
Storage gpu_unary(const Storage& s, Dtype dt, F&& f) {
    ...
    if (!g.arr) throw LucidError("gpu_unary: null GPU array");
    auto out = f(*g.arr);
...
Storage negate_storage(const Storage& s, std::size_t numel, Dtype dt, Device device) {
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vneg_f32(
                reinterpret_cast<const float*>(src.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vneg_f64(
                reinterpret_cast<const double*>(src.ptr.get()),
...
Storage multiply_storages(const Storage& a, const Storage& b,
    ...
    if (device == Device::GPU) {
        return gpu_binary(a, b, dt, [](const auto& x, const auto& y) {
            return ::mlx::core::multiply(x, y);
        });
    }
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vmul_f32(
                reinterpret_cast<const float*>(sa.ptr.get()),
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vmul_f64(
                reinterpret_cast<const double*>(sa.ptr.get()),
...
Storage divide_storages(const Storage& a, const Storage& b,
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vdiv_f32(
                reinterpret_cast<const float*>(sa.ptr.get()),
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vdiv_f64(
                reinterpret_cast<const double*>(sa.ptr.get()),
                reinterpret_cast<const double*>(sb.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
...
Storage add_storages(const Storage& a, const Storage& b,
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vadd_f32(
                reinterpret_cast<const float*>(sa.ptr.get()),
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vadd_f64(
                reinterpret_cast<const double*>(sa.ptr.get()),
                reinterpret_cast<const double*>(sb.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
...
Storage subtract_storages(const Storage& a, const Storage& b,
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vsub_f32(
                reinterpret_cast<const float*>(sa.ptr.get()),
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vsub_f64(
                reinterpret_cast<const double*>(sa.ptr.get()),
                reinterpret_cast<const double*>(sb.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
...
Storage square_storage(const Storage& s, std::size_t numel, Dtype dt, Device device) {
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vsq_f32(
                reinterpret_cast<const float*>(src.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vsq_f64(
                reinterpret_cast<const double*>(src.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
        default:
...
Storage clone_storage(const Storage& s, std::size_t numel, Dtype dt, Device device) {
    ...
    const auto& src = std::get<CpuStorage>(s);
    auto out = allocate_like(numel, dt);
    if (out.nbytes > 0) std::memcpy(out.ptr.get(), src.ptr.get(), out.nbytes);
    return Storage{std::move(out)};
}
...
Storage log_storage(const Storage& s, std::size_t numel, Dtype dt, Device device) {
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vlog_f32(
                reinterpret_cast<const float*>(src.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vlog_f64(
                reinterpret_cast<const double*>(src.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
        default:
...
Storage pow_storage(const Storage& base, const Storage& expo,
                    std::size_t numel, Dtype dt, Device device) {
    if (device == Device::GPU) {
        return gpu_binary(base, expo, dt, [](const auto& x, const auto& y) {
            return ::mlx::core::power(x, y);
        });
    }
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vpow_f32(
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<const float*>(se.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vpow_f64(
                reinterpret_cast<const double*>(sb.ptr.get()),
...
Storage ge_mask_storage(const Storage& a, const Storage& b,
    ...
    if (device == Device::GPU) {
        return gpu_binary(a, b, dt, [dt](const auto& x, const auto& y) {
            return ::mlx::core::astype(::mlx::core::greater_equal(x, y),
                                       gpu::to_mlx_dtype(dt));
        });
    }
    const auto& sa = std::get<CpuStorage>(a);
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vge_mask_f32(
                reinterpret_cast<const float*>(sa.ptr.get()),
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vge_mask_f64(
                reinterpret_cast<const double*>(sa.ptr.get()),
                reinterpret_cast<const double*>(sb.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
...
Storage lt_mask_storage(const Storage& a, const Storage& b,
    ...
    if (device == Device::GPU) {
        return gpu_binary(a, b, dt, [dt](const auto& x, const auto& y) {
            return ::mlx::core::astype(::mlx::core::less(x, y),
                                       gpu::to_mlx_dtype(dt));
        });
    }
    const auto& sa = std::get<CpuStorage>(a);
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vle_mask_f32(
                reinterpret_cast<const float*>(sa.ptr.get()),
                reinterpret_cast<const float*>(sb.ptr.get()),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vle_mask_f64(
                reinterpret_cast<const double*>(sa.ptr.get()),
                reinterpret_cast<const double*>(sb.ptr.get()),
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
...
Storage add_scalar_storage(const Storage& s, double scalar, std::size_t numel,
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vsadd_f32(
                reinterpret_cast<const float*>(src.ptr.get()),
                static_cast<float>(scalar),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vsadd_f64(
                reinterpret_cast<const double*>(src.ptr.get()), scalar,
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
        default:
...
Storage mul_scalar_storage(const Storage& s, double scalar, std::size_t numel,
    ...
    switch (dt) {
        case Dtype::F32:
            ::lucid::backend::cpu::vsmul_f32(
                reinterpret_cast<const float*>(src.ptr.get()),
                static_cast<float>(scalar),
                reinterpret_cast<float*>(out.ptr.get()), numel);
            break;
        case Dtype::F64:
            ::lucid::backend::cpu::vsmul_f64(
                reinterpret_cast<const double*>(src.ptr.get()), scalar,
                reinterpret_cast<double*>(out.ptr.get()), numel);
            break;
        default:
...
Storage apply_unary_dispatch_cpu(const Storage& s, std::size_t numel, Dtype dt,
                                 F32Fn f32, F64Fn f64, const char* op_name) {
    const auto& src = std::get<CpuStorage>(s);
    auto out = allocate_like(numel, dt);
    switch (dt) {
        case Dtype::F32:
        ...
        default:
            throw NotImplementedError(
                std::string(op_name) + ": dtype not supported");
    }
    return Storage{std::move(out)};
}
...
#define LUCID_UNARY_HELPER(NAME, F32, F64, MLX_OP)                          \
    Storage NAME##_storage(const Storage& s, std::size_t n, Dtype dt,       \
                           Device device) {                                 \
        if (device == Device::GPU) {                                        \
            return gpu_unary(s, dt, [](const auto& a) {                     \
                return ::mlx::core::MLX_OP(a);                              \
            });                                                             \
        }                                                                   \
        return apply_unary_dispatch_cpu(s, n, dt,                           \
                                        ::lucid::backend::cpu::F32,         \
                                        ::lucid::backend::cpu::F64,         \
                                        #NAME);                             \
    }
...
Storage tan_storage(const Storage& s, std::size_t numel, Dtype dt, Device device) {
    ...
    return apply_unary_dispatch_cpu(s, numel, dt,
                                    ::lucid::backend::cpu::vtan_f32,
                                    ::lucid::backend::cpu::vtan_f64,
                                    "tan");
}
    ...
    if (device == Device::GPU) {
        return gpu_unary(s, dt, [lo, hi, dt](const auto& a) {
            ...
            auto in_range = ::mlx::core::logical_and(
                ::mlx::core::greater_equal(a, lo_arr),
...
void check_version_match(const std::weak_ptr<TensorImpl>& live,
    ...
    auto t = live.lock();
    if (!t) return;  // tensor freed; nothing to compare against
...
std::vector<int> normalize_axes(const std::vector<int>& axes, int ndim) {
    ...
    if (axes.empty()) {
        out.reserve(ndim);
        for (int i = 0; i < ndim; ++i) out.push_back(i);
        return out;
    }
    ...
    for (int a : axes) {
        ...
        if (wrapped < 0 || wrapped >= ndim) {
            throw IndexError(
                std::string("axis out of range: ") + std::to_string(a) +
...
Shape reduce_output_shape(const Shape& input_shape,
    ...
    for (int a : axes) reduce_mask[a] = true;
    Shape out;
...
void unravel(std::size_t flat, const Shape& shape, std::vector<std::int64_t>& idx) {
    ...
    for (std::ptrdiff_t d = static_cast<std::ptrdiff_t>(shape.size()) - 1;
         d >= 0; --d) {
...
template <typename T>
void broadcast_back_typed(const T* g, T* out,
                          const Shape& kept_shape, const Shape& input_shape) {
    ...
    for (std::size_t flat = 0; flat < in_numel; ++flat) {
        ...
        std::size_t stride = 1;
        for (std::ptrdiff_t d = static_cast<std::ptrdiff_t>(input_shape.size()) - 1;
             d >= 0; --d) {
...
Storage broadcast_back_for_reduce(const Storage& grad,
    ...
    if (device == Device::GPU) {
        ...
        if (!g.arr) throw LucidError("broadcast_back_for_reduce: null GPU array");
        ...
        auto bcast = ::mlx::core::broadcast_to(reshaped,
                                               gpu::to_mlx_shape(input_shape));
...
Storage leaky_mask_storage(const Storage& s, double slope, std::size_t numel,
    ...
    if (device == Device::GPU) {
        return gpu_unary(s, dt, [slope, dt](const auto& a) {
            ...
            return ::mlx::core::where(::mlx::core::greater_equal(a, zero),
                                      one, slope_arr);
        });
    }
    const auto& src = std::get<CpuStorage>(s);
...
Storage bernoulli_mask_storage_shape(double keep_prob, const Shape& shape,
    ...
    switch (dt) {
        ...
        case Dtype::F64: {
            auto* q = reinterpret_cast<double*>(out.ptr.get());
            for (std::size_t i = 0; i < numel; ++i) {
                q[i] = (static_cast<double>(gen.next_uniform_float()) < keep_prob)
                           ? 1.0 : 0.0;
            }
            break;
        }
...
Storage positive_mask_storage(const Storage& s, std::size_t numel,
    ...
    if (device == Device::GPU) {
        return gpu_unary(s, dt, [dt](const auto& a) {
            ::mlx::core::array zero(0.0, gpu::to_mlx_dtype(dt));
            return ::mlx::core::astype(::mlx::core::greater(a, zero),
                                       gpu::to_mlx_dtype(dt));
        });
    }
    const auto& src = std::get<CpuStorage>(s);
    ...
    switch (dt) {
        case Dtype::F32: {
            auto* p = reinterpret_cast<const float*>(src.ptr.get());
            auto* q = reinterpret_cast<float*>(out.ptr.get());
            for (std::size_t i = 0; i < numel; ++i) q[i] = (p[i] > 0.f) ? 1.f : 0.f;
            break;
        }
        case Dtype::F64: {
            auto* p = reinterpret_cast<const double*>(src.ptr.get());
            auto* q = reinterpret_cast<double*>(out.ptr.get());
            for (std::size_t i = 0; i < numel; ++i) q[i] = (p[i] > 0.0) ? 1.0 : 0.0;
            break;
        }
        default:
...
template <typename T>
void fill_normal(T* dst, std::size_t numel, double mean, double std,
                 Generator& gen) {
...
void fill_randint(Int* dst, std::size_t numel,
                  std::int64_t low, std::int64_t high, Generator& gen) {
    const std::uint64_t range =
        static_cast<std::uint64_t>(high - low);
    if (range == 0) {
        for (std::size_t i = 0; i < numel; ++i) dst[i] = static_cast<Int>(low);
        return;
    }
    std::uint32_t buf[4];
    std::size_t i = 0;
...
Storage random_uniform_storage(const Shape& shape, double lo, double hi,
    ...
    switch (dt) {
        case Dtype::F32:
            fill_uniform<float>(reinterpret_cast<float*>(cpu.ptr.get()),
                                n, lo, hi, gen); break;
        case Dtype::F64:
            fill_uniform<double>(reinterpret_cast<double*>(cpu.ptr.get()),
                                 n, lo, hi, gen); break;
        default:
            throw NotImplementedError(
                "random_uniform: dtype not supported (F32/F64)");
    }
    if (device == Device::GPU) {
...
Storage random_normal_storage(const Shape& shape, double mean, double std,
    ...
    switch (dt) {
        case Dtype::F32:
            fill_normal<float>(reinterpret_cast<float*>(cpu.ptr.get()),
                               n, mean, std, gen); break;
        case Dtype::F64:
            fill_normal<double>(reinterpret_cast<double*>(cpu.ptr.get()),
                                n, mean, std, gen); break;
        default:
            throw NotImplementedError(
                "random_normal: dtype not supported (F32/F64)");
    }
    if (device == Device::GPU) {
        return Storage{gpu::upload_cpu_to_gpu(cpu, shape)};
    }
...
Storage random_bernoulli_storage(const Shape& shape, double p,
    ...
    switch (dt) {
        ...
        case Dtype::F64: {
            ...
            for (std::size_t i = 0; i < n; ++i) {
                q[i] = (static_cast<double>(gen.next_uniform_float()) < p)
                           ? 1.0 : 0.0;
            }
            break;
        }
        default:
            throw NotImplementedError(
                "random_bernoulli: dtype not supported (F32/F64)");
    }
    if (device == Device::GPU) {
        return Storage{gpu::upload_cpu_to_gpu(cpu, shape)};
    }
...
Storage random_randint_storage(const Shape& shape,
    ...
    switch (dt) {
        case Dtype::I32:
            fill_randint<std::int32_t>(
                reinterpret_cast<std::int32_t*>(cpu.ptr.get()),
                n, low, high, gen); break;
        case Dtype::I64:
            fill_randint<std::int64_t>(
                reinterpret_cast<std::int64_t*>(cpu.ptr.get()),
                n, low, high, gen); break;
        default:
            throw NotImplementedError(
                "random_randint: dtype not supported (I32/I64)");
    }
    if (device == Device::GPU) {
        return Storage{gpu::upload_cpu_to_gpu(cpu, shape)};
    }
