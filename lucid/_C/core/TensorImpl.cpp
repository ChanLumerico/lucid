...
Dtype np_dtype_to_lucid(const py::dtype& d) {
    ...
    switch (k) {
        ...
        case 'i':
            switch (sz) {
                case 1: return Dtype::I8;
                case 2: return Dtype::I16;
                case 4: return Dtype::I32;
                case 8: return Dtype::I64;
            }
            break;
        case 'u':
            ...
            throw DtypeMismatch(
                "signed integer or float numpy dtype",
                std::string("uint") + std::to_string(sz * 8),
                "from_numpy: unsigned dtypes are not supported "
                "(cast to int / float explicitly)");
        case 'f':
            switch (sz) {
                case 2: return Dtype::F16;
                case 4: return Dtype::F32;
                case 8: return Dtype::F64;
            }
            break;
        case 'c':
            if (sz == 8) return Dtype::C64;
            break;
        default:
            break;
    }
    throw DtypeMismatch(
        "supported numpy dtype",
...
py::dtype lucid_dtype_to_np(Dtype dt) {
    switch (dt) {
        case Dtype::Bool: return py::dtype("bool");
        case Dtype::I8:   return py::dtype("int8");
        case Dtype::I16:  return py::dtype("int16");
        case Dtype::I32:  return py::dtype("int32");
        case Dtype::I64:  return py::dtype("int64");
        case Dtype::F16:  return py::dtype("float16");
        case Dtype::F32:  return py::dtype("float32");
        case Dtype::F64:  return py::dtype("float64");
        case Dtype::C64:  return py::dtype("complex64");
    }
    throw LucidError("lucid_dtype_to_np: unknown Dtype");
}

py::object make_numpy_view(const CpuStorage& s, const Shape& shape,
                           const Stride& stride) {
    ...
    py::capsule owner(keepalive, [](void* p) {
        delete static_cast<std::shared_ptr<std::byte[]>*>(p);
    });
...
TensorImpl::TensorImpl(Storage storage, Shape shape, Dtype dtype, Device device,
                       bool requires_grad)
...
TensorImpl::from_numpy(py::array arr, Device device, bool requires_grad) {
    py::array_t<std::byte, py::array::c_style | py::array::forcecast> view =
        py::array_t<std::byte, py::array::c_style | py::array::forcecast>::ensure(
            arr);
    ...
    py::array contig = py::array::ensure(
    ...
    if (device == Device::GPU) {
        ...
        return std::make_shared<TensorImpl>(Storage{std::move(gpu)},
                                            std::move(shape), dtype, device,
                                            requires_grad);
    }
    return std::make_shared<TensorImpl>(Storage{std::move(cpu)}, std::move(shape),
    ...
    return std::visit(
        overloaded{
            [&](const CpuStorage& s) -> py::object {
                return make_numpy_view(s, shape_, stride_);
            },
...
void TensorImpl::copy_from(const TensorImpl& other) {
    ...
    if (other.dtype_ != dtype_) {
        throw DtypeMismatch(std::string(dtype_name(dtype_)),
                            std::string(dtype_name(other.dtype_)),
                            "copy_from");
    }
    ...
    std::visit(
        overloaded{
            ...
            [&](GpuStorage& dst, const GpuStorage& src) {
                ...
                dst.arr =
                    gpu::wrap_mlx_array(std::move(cloned), dst.dtype).arr;
            },
            [&](auto&, auto&) {
                throw DeviceMismatch(std::string(device_name(device_)),
                                     std::string(device_name(other.device_)),
