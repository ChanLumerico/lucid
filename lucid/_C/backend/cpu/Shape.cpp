...
template <typename T>
void permute_typed(const T* in, T* out,
    ...
    std::size_t numel = 1;
    for (auto d : out_shape) numel *= static_cast<std::size_t>(d);
    if (numel == 0) return;
...
void permute_copy_f32(const float* in, float* out,
                      const std::vector<std::int64_t>& in_shape,
                      const std::vector<int>& perm) {
    permute_typed<float>(in, out, in_shape, perm);
}

void permute_copy_f64(const double* in, double* out,
                      const std::vector<std::int64_t>& in_shape,
                      const std::vector<int>& perm) {
    permute_typed<double>(in, out, in_shape, perm);
}

void permute_copy_i32(const std::int32_t* in, std::int32_t* out,
                      const std::vector<std::int64_t>& in_shape,
                      const std::vector<int>& perm) {
    permute_typed<std::int32_t>(in, out, in_shape, perm);
}

void permute_copy_i64(const std::int64_t* in, std::int64_t* out,
                      const std::vector<std::int64_t>& in_shape,
                      const std::vector<int>& perm) {
