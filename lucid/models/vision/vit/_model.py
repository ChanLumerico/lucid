...
Architecture:
    ...
    Dropout
    N × ViTBlock:
        LayerNorm → MultiheadAttention (batch_first) → residual
...
def _build_trunk(cfg: ViTConfig) -> tuple[
    ...
    blocks = nn.ModuleList([
        _ViTBlock(cfg.dim, cfg.num_heads, mlp_dim, cfg.dropout, cfg.attention_dropout)
...
class ViTForImageClassification(PretrainedModel, ClassificationHeadMixin):
    ...
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.blocks      = blocks
        self.norm        = norm
        self._build_classifier(config.dim, config.num_classes)
    ...
    def forward(  # type: ignore[override]
        ...
        for blk in self.blocks:
            x = cast(Tensor, blk(x))
        x = cast(Tensor, self.norm(x))
        logits = cast(Tensor, self.classifier(x[:, 0]))
