...
from .util import DetectionDict
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, mask: Tensor) -> Tensor:
        ...
        not_mask = 1.0 - mask.astype(lucid.Float32)
        ...
        ).reshape(mask.shape[0], mask.shape[1], mask.shape[2], -1)
        pos_y = lucid.stack(
...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers - 1):
            self.layers.append(encoder_layer.__class__(**encoder_layer._config))
        self.norm = norm
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        tgt2 = self.multihead_attn(
            self._with_pos_embed(tgt, query_pos),
            self._with_pos_embed(memory, pos),
            memory,
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        for _ in range(num_layers - 1):
            self.layers.append(decoder_layer.__class__(**decoder_layer._config))
        self.norm = norm
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        tgt = lucid.zeros(bs, n_queries, self.d_model, dtype=src.dtype, device=src.device)
...
class HungarianMatcher(nn.Module):
    ...
    def _linear_sum_assignment(cost_matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        path = np.zeros((size * 2, 2), dtype=np.int32)

        def _step1() -> None:
            padded -= padded.min(axis=1)[:, None]
        ...
        def _step4() -> bool:
            for j in range(size):
                if np.any(mask[:, j] == 1):
                    col_cover[j] = True
            return int(col_cover.sum()) >= n_rows
        ...
        def _step6() -> None:
            ...
            if not np.any(uncovered_rows) or not np.any(uncovered_cols):
                return
            min_val = np.min(padded[np.ix_(uncovered_rows, uncovered_cols)])
    ...
    def forward(
        ...
        for b, target in enumerate(targets):
            ...
            cost_np = np.array(total_cost.to("cpu").data, dtype=np.float64)
...
class DETR(nn.Module):
    def __init__(
        ...
        self.position_embedding = PositionEmbeddingSine(backbone.num_channels // 2)
    ...
    def _get_src_permutation_idx(
        ...
        if not src_idx:
            return (
                lucid.empty(0, dtype=lucid.Int32),
                lucid.empty(0, dtype=lucid.Int32),
            )
    ...
    def forward(
        ...
        query_embed = self.query_embed.weight.unsqueeze(0).repeat(B, axis=0)
        ...
        out: dict[str, Tensor | list[dict[str, Tensor]]] = {
        ...
        if self.aux_loss:
            ...
            for layer_out in hs[:-1]:
                aux_outputs.append(
                    {
                        "pred_logits": self.class_embed(layer_out),
                        "pred_boxes": F.sigmoid(self.bbox_embed(layer_out)),
                    }
                )
    ...
    def predict(
        ...
        for b in range(x.shape[0]):
            ...
            if lucid.sum(keep.astype(lucid.Int32)).item() == 0:
                results.append([])
                continue
    ...
    def _compute_losses(
        ...
        if idx[0].size > 0:
            ...
            loss_bbox = lucid.zeros((), dtype=pred_boxes.dtype, device=pred_boxes.device)
            loss_giou = lucid.zeros((), dtype=pred_boxes.dtype, device=pred_boxes.device)
    ...
    def get_loss(
        self, x: Tensor, targets: Sequence[dict[str, Tensor]], mask: Tensor | None = None
    ) -> Tensor:
        ...
        if self.aux_loss and "aux_outputs" in outputs:
            for aux_out in outputs["aux_outputs"]:  # type: ignore[index]
                aux_losses = self._compute_losses(aux_out, targets)
                total_loss += (
...
def _build_backbone(
    name: str, train_backbone: bool = True
) -> BackboneBase:
    if name == "resnet50":
        backbone = resnet_50()
    elif name == "resnet101":
...
def detr_resnet_50(
    ...
    backbone = _build_backbone("resnet50", train_backbone=train_backbone)
    transformer = Transformer(
        d_model=256,
        nhead=8,
    ...
    return DETR(backbone, transformer, num_classes, num_queries=num_queries)
...
def detr_resnet_101(
    ...
    backbone = _build_backbone("resnet101", train_backbone=train_backbone)
    transformer = Transformer(
        d_model=256,
        nhead=8,
        num_encoder_layers=6,
