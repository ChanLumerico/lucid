...
class _SlowROIPool(nn.Module):
    ...
    def forward(self, feature: Tensor, rois: Tensor, roi_idx: Tensor) -> Tensor:
        N = rois.shape[0]
        H, W = feature.shape[2:]
        ...
        for i in range(N):
            ...
            patch = feature[b : b + 1, :, yi1:yi2, xi1:xi2]
        ...
        if not crops:
            return lucid.empty(0, feature.shape[1], ph, pw, device=feature.device)
...
class _AnchorGenerator(nn.Module):
    ...
    def grid_anchors(self, feat_h: int, feat_w: int, device: str) -> Tensor:
